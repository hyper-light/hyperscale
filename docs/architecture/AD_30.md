...
## Addendum (Phase C) — Suspicion-timer composition

The earlier shorthand `T = k × log(n) × LHM` (state-machine diagrams in
`architecture.md:5196` and the failure-detection probe diagram at
`architecture.md:5734`) leaves the **source** of `LHM` unspecified.
This addendum nails that down.

The suspicion-timer bracket `(min_timeout, max_timeout)` passed into
the Lifeguard `calculate_timeout` formula is composed multiplicatively
from **independent, bounded factors** per the AD-35:186 pattern:

```
suspicion_bracket = (global_min_timeout, global_max_timeout)
                    × self_lhm
                    × peer_load
                    × vivaldi_quality
```
...
* **`self_lhm`** — `LocalHealthMultiplier.get_multiplier()`, range
  `[1.0, 3.0]` per `architecture.md:7221`. Captures *prober's*
  self-health (slow event loop, missed nacks, etc.).

* **`peer_load`** — `PeerHealthAwareness.get_load_multiplier(node)`,
  range `[1.0, 2.5]` per `PeerHealthAwarenessConfig`. Captures the
  *target's* reported overload state from AD-19 health gossip.
  Returns `1.0` when the peer is unknown or stale (deliberately
  neutral — peer-going-dark must not extend the very timer that
  detects it).

* **`vivaldi_quality`** — `1.0 + vivaldi_error/10.0` per AD-35:183,
  range `[1.0, 1.5]`. Captures uncertainty about the predicted
  network distance to the target. Returns `1.0` when no peer
  coordinate is tracked yet.

* The Lifeguard confirmation/cluster logarithmic term
  `log(c+1)/log(n+1)` is applied by `SuspicionState.calculate_timeout`
  on top of the composed bracket — unchanged from the original
  AD-30 design.

**Worst-case bound:** `max_timeout × 3.0 × 2.5 × 1.5 = 11.25 ×
max_timeout` when self-LHM saturated, peer reported OVERLOADED, and
Vivaldi error very high. With `global_max_timeout = 30s`, that's
~338s — extreme but bounded. Typical case (self-LHM ≈ 1.5, peer
healthy, Vivaldi quality ≈ 1.05) gives ~1.6× — a small budget
overhead for paper-correct adaptive scaling.

## Addendum (Phase C) — Self-LHM growth gating

`increase_failure_detector("probe_timeout")` is gated by the target's
state in the incarnation tracker. When the target is already SUSPECT
or DEAD, further probe-timeouts to that target **do not** pump
self-LHM.

**Rationale.** Lifeguard's LHM tracks *self*-slowness — missed nacks,
event-loop lag, refutations against the local node. Probe failures
to a peer we've already concluded is in trouble are evidence of the
*peer's* deadness, not of our slowness. Without the gate, a single
dead peer creates a positive-feedback loop on small clusters: probe
timeouts → self-LHM increment → longer suspicion bracket via the
multiplicative composition above → more probe rounds before DEAD →
self-LHM increment again. Detection gets *slower* as the cluster
gets *unhealthier* — opposite of SWIM's intent.

The gate reads `IncarnationTracker.get_node_state(target).status` per
AD-46. Unknown peers (no tracker entry) are treated as evidence-of-
self-slowness on probe-timeout, since we may simply have failed to
establish baseline communication with them.
