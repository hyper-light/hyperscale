...
## Addendum: AD-26 H7/H8 Persistence Integration

`TimeoutTrackingState` was extended to carry the AD-26 Phase H7/H8
extension-protocol state through leader transfers. This is the
companion mechanism to AD-48 `#|x` / `#|o` dissemination —
gossip handles the steady-state convergence; this state-sync
path handles the leader-takeover instant where the new leader
needs the previous leader's view immediately.

```
TimeoutTrackingState (additions)
├─ last_progress_snapshots:    dict[workflow_id, WorkflowProgressSnapshot]
│   └─ Most-recent H3 snapshot per active workflow
├─ last_extension_decisions:   dict[workflow_id, ExtensionDecisionEvent]
│   └─ Most-recent H7 decision per active workflow
├─ pending_extension_outcomes: dict[workflow_id, ExtensionOutcomeEvent]
│   └─ H8 termination outcomes whose #|o gossip may not have
│      reached every peer yet
└─ alpha_tuner_snapshot:       dict[workflow_class, bytes]
    └─ Frozen Beta(α, β) posteriors for the H8 Bayesian tuner
```

On leader takeover, the new leader calls
`WorkerHealthManager.replay_persisted_state(state)` to:

1. Restore the alpha-tuner snapshot (authoritative tuner state at
   the previous leader's last persistence point).
2. Replay decisions and outcomes into the ledger — but NOT into
   the tuner (the snapshot already reflects them, applying them
   again would double-count Bernoulli observations).

This guarantees no extension-history regression and no
posterior-mean regression across leader transfers.
