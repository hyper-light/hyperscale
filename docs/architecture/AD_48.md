...
## Part 16: AD-26 H7b/H8b Channel Extensions

The AD-48 piggyback multiplexer originally carried four channels:
`#|m` (membership), `#|h` (health), `#|w` (worker state), and
`#|v` (Vivaldi). AD-26 Phase H added two more channels using the
same dissemination discipline (λ × log(n+1) broadcast budget,
incarnation-style supersession, idempotent dedup).

### H7b: Extension decision channel — `#|x`

Carries `ExtensionDecisionEvent` records — every grant or denial
of a healthcheck extension, with the full witness evidence. Wire
format is 29 colon-delimited fields with the trailing slot
reserved for `denial_message` (the only variable-content field).

Dedup key: `event_id = workflow_id|fence_token|timestamp` — the
triple is unique because leader_term advances monotonically and
timestamps are the leader's monotonic clock. Higher-leader-term
events supersede existing slots.

`ExtensionDecisionGossipBuffer` mirrors `WorkerStateGossipBuffer`
exactly. `ManagerServer` is the only `HealthAwareServer` subclass
that overrides `_get_extension_decision_piggyback` /
`_process_extension_decision_piggyback` — workers and gates pass
the channel through transparently.

### H8b: Extension outcome channel — `#|o`

Carries `ExtensionOutcomeEvent` records — workflow-termination
training signals for the H8 Bayesian alpha tuner. Wire format is
13 colon-delimited fields with `workflow_class` as the trailing
free-form slot.

Dedup key: `event_id = workflow_id` alone — a workflow has at
most one outcome (it terminates exactly once).

### Strip order

The receiver-side parser walks right-to-left through the layers
on every inbound message:

```
#|v (vivaldi)  ->  #|o (outcome)  ->  #|x (decision)  ->
#|w (worker)   ->  #|h (health)   ->  #|m (membership)
```

Each layer slices its piggyback frame out and updates `msg_end`
so the next layer sees a clean residual. New piggyback layers
must always be added between the existing layers and `#|v` on
the encode side, and stripped right after `#|v` on the decode
side, to preserve binary compatibility with existing peers.
