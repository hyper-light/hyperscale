...
class HARConnector:
    ...
    async def connect(self):
        ...
        self.filepath = await self._loop.run_in_executor(
            None, functools.partial(os.path.abspath, self.filepath)
        )

        self.har_file = await self._loop.run_in_executor(
            None, functools.partial(open, self.filepath)
        )
        ...
        for signame in ("SIGINT", "SIGTERM", "SIG_IGN"):
            self._loop.add_signal_handler(
                ...
                lambda signame=signame: handle_loop_stop(
                    signame, self._executor, self._loop, self.har_file
                ),
            )
    ...
    async def load_execute_stage_summary(
        ...
        raise NotImplementedError(
            "Execute stage summary loading is not available for the HAR Connector."
        )

    async def load_actions(
        self, options: Dict[str, Any] = {}
    ) -> Coroutine[Any, Any, List[ActionHook]]:
        self._parser = HarParser(har_data=json.loads(self.har_file))
        ...
        for page in self._parser.pages:
            ...
            for entry in page_entries:
                ...
                action_basename = "_".join(
                    [
                        segment.capitalize()
                        for segment in self._name_pattern.sub("_", action_url).split(
                            "_"
                        )
                    ]
                )
    ...
    async def load_results(
        ...
        raise NotImplementedError(
            "Results loading is not available for the HAR Connector."
        )
    ...
    async def load_data(
        ...
        raise NotImplementedError(
            "Data loading is not available for the HAR Connector."
        )

    async def close(self):
        await self._loop.run_in_executor(self._executor, self.har_file.close)
