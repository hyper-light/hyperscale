...
class CassandraConnector:
    ...
    def __init__(
        ...
        self._columns_factory: Dict[str, Callable[[Dict[str, Any]], columns.Column]] = {
            ...
            "tuple": lambda column_config: columns.Tuple(**column_config),
    ...
    async def create_schemas(self):
        for schema in self.schemas.action_schemas(self.table_name):
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(sync_table, schema, keyspaces=[self.keyspace]),
            )
    ...
    async def load_data(
        ...
        if table is None:
            fields = {
                field_name: self._columns_factory.get(field_config.field_type)(
                    field_config.options
                )
                for field_name, field_config in cassandra_load_request.fields.items()
            }

            table = type(table_name, (Model,), fields)

        if cassandra_load_request.filters:
            data_rows: ModelQuerySet = await self._loop.run_in_executor(
                self._executor,
                functools.partial(table.filter, **cassandra_load_request.filters),
            )

        else:
            data_rows: ModelQuerySet = await self._loop.run_in_executor(
                self._executor, table.all
            )

        if cassandra_load_request.limit:
            data_rows = await self._loop.run_in_executor(
                self._executor,
                functools.partial(data_rows.limit, cassandra_load_request.limit),
            )

        return [row for row in data_rows]

    async def store_actions(
        self, actions: List[Action], options: Dict[str, Any] = {}
    ) -> Coroutine[Any, Any, List[Dict[str, Any]]]:
        pass
    ...
    async def load_actions(
        ...
        tables = [table for table in self.schemas.action_schemas(self.table_name)]
        ...
        table_results: List[List[Dict[str, Any]]] = await asyncio.gather(
            *[
                asyncio.create_task(
                    self.load_data(
                        options={"table": table, "table_name": table_name, **options}
                    )
                )
