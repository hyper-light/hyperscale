...
from hyperscale.core.engines.types.udp.action import UDPAction
...
class CassandraConnector:
    ...
    async def connect(self):
        ...
        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Opening amd authorizing connection to Cassandra Cluster at - {host_port_combinations}')
        await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Opening session - {self.session_uuid}')
        ...
        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Connected to Cassandra Cluster at - {host_port_combinations}')

        if self.keyspace is None:
            self.keyspace = 'hyperscale'

        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Creating Keyspace - {self.keyspace}')
        ...
        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Created Keyspace - {self.keyspace}')
