...
class CassandraSchemaSet:
    def __init__(self) -> None:
        self._action_schemas: Dict[
            ...
            RequestTypes.GRAPHQL: lambda table_name: CassandraGraphQLActionSchema(
                table_name
            ),
            RequestTypes.GRAPHQL_HTTP2: lambda table_name: CassandraGraphQLHTTP2ActionSchema(
                table_name
            ),
            ...
            RequestTypes.HTTP2: lambda table_name: CassandraHTTP2ActionSchema(
                table_name
            ),
            RequestTypes.HTTP3: lambda table_name: CassandraHTTP3ActionSchema(
                table_name
            ),
            RequestTypes.PLAYWRIGHT: lambda table_name: CassandraPlaywrightActionSchema(
                table_name
            ),
            ...
            RequestTypes.WEBSOCKET: lambda table_name: CassandraWebsocketActionSchema(
                table_name
            ),
        }
        ...
        self._results_schemas: Dict[
            ...
            RequestTypes.GRAPHQL: lambda table_name: CassandraGraphQLResultSchema(
                table_name
            ),
            RequestTypes.GRAPHQL_HTTP2: lambda table_name: CassandraGraphQLHTTP2ResultSchema(
                table_name
            ),
            ...
            RequestTypes.HTTP2: lambda table_name: CassandraHTTP2ResultSchema(
                table_name
            ),
            RequestTypes.HTTP3: lambda table_name: CassandraHTTP3ResultSchema(
                table_name
            ),
            RequestTypes.PLAYWRIGHT: lambda table_name: CassandraPlaywrightResultSchema(
                table_name
            ),
            ...
            RequestTypes.WEBSOCKET: lambda table_name: CassandraWebsocketResultSchema(
                table_name
            ),
        }

    def get_action_schema(self, request_type: RequestTypes, table_name: str):
        return self._action_schemas.get(request_type, CassandraHTTPActionSchema)(
            table_name
        )

    def get_result_schema(self, request_type: RequestTypes, table_name: str):
        return self._results_schemas.get(request_type, CassandraHTTPResultSchema)(
            table_name
        )

    def action_schemas(self, table_name: str):
    ...
    def results_schemas(self, table_name: str):
