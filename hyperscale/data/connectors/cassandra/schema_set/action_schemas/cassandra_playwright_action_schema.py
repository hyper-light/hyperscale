...
from hyperscale.core.engines.types.playwright.command import PlaywrightCommand
...
class CassandraPlaywrightActionSchema:
    def __init__(self, table_name: str) -> None:
        ...
        self.actions_table = type(
            f"{table_name}_playwright", (Model,), self.action_columns
        )
        ...
        self._serialize_map: Dict[
            str, Callable[[Type[Union[str, bytes, int, float, list, dict]]], str]
        ] = {
            str: str,
            bytes: str,
            int: str,
        ...
        self._deserialize_map: Dict[
            str, Callable[[str], Type[Union[str, bytes, int, float, list, dict]]]
        ] = {
            "string": str,
            "integer": int,
            "float": float,
    ...
    def to_schema_record(self, action: PlaywrightCommand) -> Dict[str, Any]:
        action_option: Union[Dict[str, str], None] = None
        if action.input.option:
            action_option = {
                "data": self._serialize_map.get(type(action.input.option))(
                    action.input.option
                ),
                "datatype": self._types_map.get(type(action.input.option)),
            }
        ...
        return {
            ...
            "args": [
                {
                    "data": self._serialize_map.get(type(arg))(arg),
                    "datatype": self._types_map.get(type(arg)),
                }
            ...
            "tags": [
                {
                    ...
                    "datatype": self._types_map.get(type(tag.get("value"))),
                }
                for tag in action.metadata.tags
            ],
        }

    def from_schema_record(self, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        option: Union[str, bytes, int, float, bool, list, dict, None] = None

        if action_option:
            option = self._deserialize_map.get(action_option.get("datatype"))(
        ...
        return {
            ...
            "args": [
                self._deserialize_map.get(arg_config.get("datatype"))(
                    arg_config.get("data")
                )
            ...
            "extra": json.loads(action.get("extra", {})),
