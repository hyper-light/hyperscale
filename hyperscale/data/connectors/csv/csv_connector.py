...
from typing import Any, Coroutine, Dict, List, TextIO
...
class CSVConnector:
            ...
            for signame in ("SIGINT", "SIGTERM", "SIG_IGN"):
                self._loop.add_signal_handler(
                    ...
                    lambda signame=signame: handle_loop_stop(
                        signame, self._executor, self._loop, self.csv_file
                    ),
                )
    ...
    async def load_data(
        ...
        if self._csv_reader is None:
            self._csv_reader = csv.DictReader(self.filepath, fieldnames=headers)

        return await self._loop.run_in_executor(self._executor, self._load_data)

    def _load_data(self):
        return [row for row in self._csv_reader]

    async def close(self):
        await self._loop.run_in_executor(self._executor, self.csv_file.close)
