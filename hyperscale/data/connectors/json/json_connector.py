...
class JSONConnector:
            ...
            for signame in ("SIGINT", "SIGTERM", "SIG_IGN"):
                self._loop.add_signal_handler(
                    ...
                    lambda signame=signame: handle_loop_stop(
                        signame, self._executor, self._loop, self.json_file
                    ),
                )
    ...
    async def load_data(self, options: Dict[str, Any] = {}) -> Coroutine[Any, Any, Any]:
        return await self._loop.run_in_executor(
            self._executor, functools.partial(json.load, self.json_file)
        )

    async def close(self):
        await self._loop.run_in_executor(self._executor, self.json_file.close)
