...
from hyperscale.core.engines.types.playwright.client import (
    ContextConfig,
    MercuryPlaywrightClient,
)
from hyperscale.core.engines.types.playwright.command import (
    URL,
    Input,
    Options,
    Page,
...
from hyperscale.core.engines.types.playwright.result import PlaywrightResult
...
class PlaywrightSerializer(BaseSerializer):
    ...
    def deserialize_action(self, action: Dict[str, Any]) -> PlaywrightCommand:
    ...
    def deserialize_client_config(
        ...
        playwright_client.config = ContextConfig(**client_config.get("context_config"))

        return playwright_client

    def result_to_serializable(self, result: PlaywrightResult) -> Dict[str, Any]:
    ...
    def deserialize_result(self, result: Dict[str, Any]) -> PlaywrightResult:
        playwright_command = PlaywrightCommand(
            result.get("name"),
            result.get("command"),
            page=Page(
            ...
            url=URL(location=result.get("url"), headers=result.get("headers")),
            input=Input(
                key=result.get("key"),
        ...
        playwright_result = PlaywrightResult(
            playwright_command, error=result.get("error")
        )
