...
class PlaywrightActionParser(BaseParser):
    def __init__(self, config: Config, options: Dict[str, Any] = {}) -> None:
        super().__init__(
            PlaywrightActionParser.__name__, config, RequestTypes.PLAYWRIGHT, options
        )
    ...
    async def parse(
        ...
        generator_action = PlaywrightActionValidator(
            ...
            url=PlaywrightURLValidator(
                location=action_data.get("location"), headers=normalized_headers
            ),
        ...
        action = PlaywrightCommand(
            ...
            url=URL(**generator_action.url.dict(exclude_none=True)),
            page=Page(**generator_action.page.dict(exclude_none=True)),
            input=Input(**generator_action.input.dict(exclude_none=True)),
            options=Options(**generator_action.options.dict(exclude_none=True)),
