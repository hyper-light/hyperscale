...
class HyperscaleLogger:
    def __init__(self) -> None:
        self.loggers: Dict[
            str,
            Logger[
                Union[AsyncLogger, AsyncFilesystemLogger, AsyncSpinner],
                Union[SyncLogger, SyncFilesystemLogger],
            ],
        ] = {}
    ...
    def initialize(self, config: LoggingConfig = LoggingConfig()):
        for logger_type in self.logger_types:
            ...
            if logger_type == LoggerTypes.CONSOLE:
                ...
                logger_type == LoggerTypes.DISTRIBUTED
                or logger_type == LoggerTypes.HYPERSCALE
            ):
                logger.set_patterns(
    ...
    def __getitem__(
        ...
        if logger is None:
            logger = Logger(
                logger_name, LoggerTypes.CONSOLE, log_level=logging_manager.log_level
            )
    ...
    @property
    def hyperscale(self) -> Logger[AsyncLogger, SyncLogger]:
        return self.loggers["hyperscale"]
    ...
    @property
    def distributed_filesystem(
        self,
    ) -> Logger[AsyncFilesystemLogger, SyncFilesystemLogger]:
