...
class ExecutionSummaryTable:
    def __init__(self, execution_results: ExecutionResults) -> None:
        ...
        self.session_metrics: List[str] = ["total", "succeeded", "failed"]
    ...
    def generate_tables(self):
        ...
        for table_name, table_rows in self.actions_and_tasks_table_rows.items():
            self.actions_and_tasks_table_rows[table_name] = list(
                sorted(table_rows, key=lambda row: row["name"])
            )
        ...
        self.actions_and_tasks_common_table_rows = list(
            sorted(
                self.actions_and_tasks_common_table_rows, key=lambda row: row["name"]
            )
        )
    ...
    def show_tables(self):
        ...
        if self._has_streamed and self.enabled_tables.get("stages"):
            completion_rates: List[CompletionRateSet] = list(
                sorted(
                    self.stage_streamed_data.get("completion_rates").items(),
            ...
            for stage_name, stage_completion_rates in completion_rates:
                ...
                scatter_plot = plotille.scatter(
                    ...
                    y_min=0,
                    x_min=0,
                    x_max=int(round(stage_summary.stage_metrics.time, 0)),
                    linesep="\n",
                    X_label="time (sec)",
        ...
        if self.enabled_tables.get("actions"):
            ...
            self.logger.console.sync.info(
                f"""{self.actions_and_tasks_common_table}\n"""
            )
    ...
    def _generate_stage_and_session_tables(self):
        ...
        for stage_summary in self.execution_results.values():
            ...
            if stage_summary.stage_streamed_analytics:
                ...
                self.stage_streamed_data["completed"][
                    stage_summary.stage_metrics.name
                ] = stage_summary.stage_metrics.streamed_completed
                self.stage_streamed_data["succeeded"][
                    stage_summary.stage_metrics.name
                ] = stage_summary.stage_metrics.streamed_succeeded
                self.stage_streamed_data["failed"][stage_summary.stage_metrics.name] = (
                    stage_summary.stage_metrics.streamed_failed
                )
                self.stage_streamed_data["completion_rates"][
                    stage_summary.stage_metrics.name
                ] = stage_summary.stage_metrics.streamed_completion_rates
    ...
    def _generate_actions_and_tasks_table(self):
        for stage_name, stage_metrics in self.execution_results.items():
            for (
                action_or_task_name,
                group_metrics_set,
            ) in stage_metrics.action_and_task_metrics.items():
                table_row = OrderedDict()
                ...
                for group in stage_metrics.groups:
                    ...
                    for field_name, field_value in metrics.items():
                        header_name = field_name.replace(f"{group}_", "").replace(
                            "_", " "
                        )
