...
from hyperscale.reporting.experiment.experiment_metrics_set import ExperimentSummary
...
class SummaryTable:
    def __init__(
        ...
        self.system_summary_table.enabled_tables.update(
            {
                table_name: enabled
                for table_name, enabled in summaries_visibility_config.items()
                if table_name in self.system_summary_table.enabled_tables
            }
        )

        graph_execution_results: GraphExecutionResults = graph_results.get("metrics")
        submit_stage_system_metrics: SystemMetricsCollection = graph_results.get(
            "submit_stage_system_metrics", {}
        )
        graph_system_metrics: SystemMetricsSet = graph_results.get(
        ...
        for results_set in graph_execution_results.values():
            ...
            for experiment_name, experiment in experiment_metrics_sets.items():
                ...
                if (
                    len(experiment_headers_keys["variants_stats_table_header_keys"])
                    == 0
                ):
        ...
        self.execution_summary_table.enabled_tables.update(
            {
                table_name: enabled
                for table_name, enabled in summaries_visibility_config.items()
                if table_name in self.execution_summary_table.enabled_tables
            }
        )

        if len(experiment_summaries) > 0:
            self.experiment_summary_table = ExperimentsSummaryTable(
                experiment_summaries, experiment_headers, experiment_headers_keys
            )
            ...
            self.experiment_summary_table.enabled_tables.update(
                {
                    table_name: enabled
                    for table_name, enabled in summaries_visibility_config.items()
                    if table_name in self.experiment_summary_table.enabled_tables
                }
            )
