...
class Logger(Generic[A, S]):
    def __init__(self, config: Any) -> None:
        ...
        if config.logger_type == LoggerTypes.SPINNER:
            self.aio: A = self.logger_types.async_loggers.get(
                config.logger_type, AsyncSpinner
            )(**config.spinner)
            self.sync: S = None

        elif (
            config.logger_type == LoggerTypes.FILESYSTEM
            or config.logger_type == LoggerTypes.DISTRIBUTED_FILESYSTEM
        ):
            self.aio: A = self.logger_types.async_loggers.get(
                config.logger_type, AsyncFilesystemLogger
            )(**config.filesystem_logger)
            self.sync: S = self.logger_types.sync_loggers.get(
                config.logger_type, SyncFilesystemLogger
            )(**config.filesystem_logger)

        else:
            self.aio: A = self.logger_types.async_loggers.get(
                config.logger_type, AsyncLogger
            )(**config.cli_logger)
            self.sync: S = self.logger_types.sync_loggers.get(
                config.logger_type, SyncLogger
            )(**config.cli_logger)
    ...
    def set_patterns(self, pattern: str, datefmt_pattern: str = None):
        if isinstance(self.aio, AsyncLogger) and isinstance(self.sync, SyncLogger):
            self.aio.initialize(pattern, datefmt_pattern=datefmt_pattern)

            self.sync.initialize(pattern, datefmt_pattern=datefmt_pattern)

    def create_filelogger(self, filepath: str):
        if isinstance(self.aio, AsyncFilesystemLogger) and isinstance(
            self.sync, SyncFilesystemLogger
        ):
