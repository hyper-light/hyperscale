...
from hyperscale.terminal.components.spinner.spinner_config import SpinnerConfig
...
from hyperscale.terminal.components.spinner.spinner_types import SpinnerName
...
async def default_handler(signame: str, spinner: ProgressBar):  # pylint: disable=unused-argument
...
class ProgressBar:
    ...
    async def _spin(
        ...
        while not self._stop_spin.is_set():
            ...
            spin_phase = [next(self._cycle) for _ in range(size)]
            ...
            if sig and sig_handler:
                self._loop.add_signal_handler(
                    ...
                    lambda signame=sig.name: asyncio.create_task(
                        asyncio.to_thread(sig_handler, signame, self)
                    ),
                )
