...
def _type(
    ...
    if string is None:
        return lambda _: missingval if isinstance(missingval, str) else str(missingval)
...
def _format(val, valtype, floatfmt, intfmt, missingval="", has_invisible=True):
    ...
    if valtype is str:
        ...
        return format(val, intfmt) if intfmt is not None else str(val)
        ...
        if is_a_colored_number:
            ...
            formatted_val = (
                format(float(raw_val), floatfmt)
                if floatfmt is not None
                else str(float(raw_val))
            )
    ...
    for row in list_of_lists:
        ...
        for cell, width, numparse in zip(row, colwidths, numparses):
            ...
            if width is not None:
                ...
                if casted_cell is None:
                    casted_cell = str(casted_cell)
