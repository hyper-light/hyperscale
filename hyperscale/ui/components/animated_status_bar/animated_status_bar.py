...
class AnimatedStatusBar:
    ...
    async def _rerender(
        ...
        if (
            cached_frame := self._precomputed_frames.get((
                status, 
    ...
    async def _precompute_status_styles(self):
        for status, status_styles in self._status_styles.items():
            ...
            style_strings = [stylizer for stylizer in status_styles.values() if isinstance(stylizer, str)]
    ...
    async def _generate_status_precomputed_frames(self, status: str):
            ...
            for _ in range(animation_length):
                cache_key = (status, self._current_direction, self._active_segment, self._active_idx)
                frame = await self._rerender(status)
    ...
    async def _apply_swipe_animation(
        ...
        elif self._current_direction == 'forward' and self._active_segment == 'inactive':
    ...
    async def _apply_color_or_highlight_animation(
        self,
        status: str
    ):
            ...
            inactive_text = await stylize(
                ...
                attrs=[
                    get_style(
                        ...
                        'primary_attrs',
                        status_styles.get('secondary_attrs', [])
                    )
                ],
                mode=self._mode,
            )
