...
def assemble_exanded_args(args: list[str]):
    ...
    for arg in args:
        if arg.startswith('--'):
            ...
            expanded_args = [
                f'-{short_arg}' for short_arg in list(arg.strip('-'))
            ]
...
def inspect_wrapped(
    ...
    for arg_name, arg_attrs in call_args.parameters.items():
        ...
        if arg_attrs.default == inspect._empty and arg_attrs.annotation == inspect._empty:
            raise Exception(f'Err. - cannot use unannotated arg {arg_name} for command signature')
