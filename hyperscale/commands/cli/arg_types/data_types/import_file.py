    ...
    async def _import_file_objects(self, arg: str |None):
        try:
            ...
            for conversion_type in self._types:
                ...
                discovered = list({cls.__name__: cls for cls in conversion_type.__subclasses__()}.values())

                imported_types.update({
                    item.__name__: item for item in discovered
                })

            return imported_types
            
        except Exception as e:
            return Exception(f'encountered error {str(e)} importing objects from file {arg}')
