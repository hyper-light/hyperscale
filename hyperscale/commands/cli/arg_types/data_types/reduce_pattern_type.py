...
def _concat_pattern_types(
    pattern: Any
):
    if isinstance(pattern, tuple) and len(pattern) > 0:        
        return [
            _concat_pattern_types(arg) for arg in pattern
        ]
        ...
        return [
            _concat_pattern_types(arg) for arg in args
        ]
...
def reduce_pattern_type(pattern: Any):
    try:
        ...
        return functools.reduce(
            ...
            results,
            []
        ) if len(results) > 0 and isinstance(results[0], list) else results
