...
class ExperimentsSummaryTable:
    ...
    def _generate_experiments_table(self) -> str:
        ...
        return tabulate(
            list(sorted(experiment_table_rows, key=lambda row: row["name"])),
            headers="keys",
            missingval="None",
            tablefmt="simple",
            floatfmt=".2f",
    ...
    def _generate_variants_tables(self) -> Dict[str, Union[str, None]]:
        variants_table_header_keys = self.experiment_header_keys.get(
            "variant_table_headers_keys"
        )
        variants_stats_table_header_keys = self.experiment_header_keys.get(
        ...
        for experiment_summary in self.experiments_summaries.values():
            for (
                variant_summary
            ) in experiment_summary.experiment_variant_summaries.values():
                table_row = OrderedDict()
    ...
    def _generate_mutations_table(self) -> str:
        ...
        mutations_table_headers_keys = self.experiment_header_keys.get(
        ...
        for experiment_summary in self.experiments_summaries.values():
            for (
                ...
                for (
                    mutation_summary
                ) in variant_summary.variant_mutation_summaries.values():
        ...
        return tabulate(
            list(sorted(mutation_table_rows, key=lambda row: row["name"])),
            headers="keys",
            missingval="None",
            tablefmt="simple",
        )
