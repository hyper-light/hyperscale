...
class SystemSummaryTable:
    def __init__(self) -> None:
        ...
        self.enabled_tables = {"system": False}
    ...
    def show_tables(self):
        ...
        if self.enabled_tables.get("system"):
            for (
                stage_name,
                monitor,
            ) in self.graph_metrics_summary.cpu.stage_metrics.items():
                for monitor_name, metrics in monitor.items():
                    show_plot = self.graph_metrics_summary.cpu.visibility_filters[
                        stage_name
                    ][monitor_name]

                    if show_plot:
                        scatter_plot = plotille.scatter(
                            [idx for idx in range(1, len(metrics) + 1)],
                            metrics,
                            width=120,
                            height=10,
                            y_min=0,
            ...
            for (
                stage_name,
                monitor,
            ) in self.graph_metrics_summary.memory.stage_metrics.items():
                for monitor_name, metrics in monitor.items():
                    show_plot = self.graph_metrics_summary.memory.visibility_filters[
                        stage_name
                    ][monitor_name]
        ...
        if self.enabled_tables.get("system") and self.enabled_tables.get("stages"):
            ...
            for metrics_set in self.system_metrics_summaries:
                for stage_name, monitor in metrics_set.cpu.stage_metrics.items():
                    for monitor_name, metrics in monitor.items():
                        show_plot = metrics_set.cpu.visibility_filters[stage_name][
                            monitor_name
                        ]
    ...
    def _to_cpu_table(self):
        ...
        return tabulate(
            list(sorted(table_rows, key=lambda row: row["name"])),
            headers="keys",
            missingval="None",
            tablefmt="simple",
            floatfmt=".2f",
