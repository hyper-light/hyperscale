...
from hyperscale.distributed.env import Env, ReplicationEnv, load_env
...
from hyperscale.distributed.models.raft import (
    ElectionState,
    HealthCheck,
    RaftMessage,
    VoteResult,
)
from hyperscale.distributed.models.raft.logs import Entry, NodeState
from hyperscale.distributed.monitoring import Monitor
from hyperscale.distributed.snowflake.snowflake_generator import (
    Snowflake,
    SnowflakeGenerator,
)
...
class ReplicationController(Monitor):
    def __init__(
        ...
        self._models = [HealthCheck, RaftMessage]

        self._term_number = 0
        self._term_votes = defaultdict(lambda: defaultdict(lambda: 0))

        self._max_election_timeout = TimeParser(
            replication_env.MERCURY_SYNC_RAFT_ELECTION_MAX_TIMEOUT
        ).time

        self._min_election_timeout = max(self._max_election_timeout * 0.5, 1)
        ...
        self._election_timeout = random.uniform(
            self._min_election_timeout, self._max_election_timeout
        )
    ...
    async def start(self):
        ...
        await self._logger.distributed.aio.info(
            f"Starting server for node - {self.host}:{self.port} - with id - {self._instance_id}"
        )
    ...
    async def register(self, host: str, port: int):
        ...
        self._raft_monitor_task = asyncio.create_task(self._run_raft_monitor())
    ...
    async def _run_registration(self):
        ...
        while self._check_all_nodes_registered() is False:
            monitors = [address for address in self._node_statuses.keys()]
            ...
            if registered_count > last_registered_count:
                await self._logger.distributed.aio.info(
                    f"Source - {self.host}:{self.port} - reporting - {registered_count}/{self._initial_expected_nodes} - as fully registered"
                )
                await self._logger.filesystem.aio[
                    f"hyperscale.distributed.{self._instance_id}"
                ].info(
            ...
            if active_nodes_count > 0:
                for host, port in monitors:
                    self._tasks_queue.append(
                        asyncio.create_task(
                            asyncio.wait_for(
                                self._submit_registration(host, port),
    ...
    def _calculate_all_registered_nodes(self) -> int:
        ...
        return len(
            [
                count
                for count in self._registered_counts.values()
                if count == self._initial_expected_nodes
            ]
        )
    ...
    async def _submit_registration(self, host: str, port: int):
        shard_id, response = await self.submit_registration(host, port)
    ...
    async def receive_vote_request(
        self, shard_id: int, raft_message: RaftMessage
    ) -> Call[RaftMessage]:
        ...
        if term_number > self._term_number:
            ...
            and self._raft_node_status != NodeState.LEADER
        ):
            ...
            return RaftMessage(
                ...
                elected_leader=(leader_host, leader_port),
                status=self.status,
        ...
        return RaftMessage(
            ...
            source_host=self.host,
            source_port=self.port,
            elected_leader=(elected_host, elected_port),
            status=self.status,
    ...
    async def receive_log_update(
        self, shard_id: int, message: RaftMessage
    ) -> Call[RaftMessage]:
        ...
        entries: List[Entry] = list(
            sorted(
                message.entries,
                key=lambda entry: Snowflake.parse(entry.entry_id).timestamp,
            )
        )
        ...
        try:
            if message.term_number > self._term_number:
                self._tasks_queue.append(
                    asyncio.create_task(self._cancel_election(message))
                )

                amount_behind = max(message.term_number - self._term_number - 1, 0)
                ...
                for _ in range(amount_behind):
                    self._term_leaders.append((None, None))

                await self._logger.distributed.aio.info(
                    f"Term number for source - {self.host}:{self.port} - was updated to - {self._term_number} - and leader was updated to - {leader_host}:{leader_port}"
                )
                await self._logger.filesystem.aio[
                    f"hyperscale.distributed.{self._instance_id}"
                ].info(
                ...
                self._term_leaders.append((leader_host, leader_port))
            ...
            if message.failed_node and self._suspect_tasks.get(message.failed_node):
                node_host, node_port = message.failed_node

                self._tasks_queue.append(
                    asyncio.create_task(
                        self._cancel_suspicion_probe(node_host, node_port)
                    )
                )

                await self._logger.distributed.aio.debug(
                    f"Node - {node_host}:{node_port} - submitted healthy status to source - {self.host}:{self.port} - and is no longer suspect"
                )
                await self._logger.filesystem.aio[
                    f"hyperscale.distributed.{self._instance_id}"
                ].debug(
            ...
            if self._suspect_tasks.get((source_host, source_port)):
                ...
                await self._logger.distributed.aio.debug(
                    f"Node - {source_host}:{source_port} - submitted healthy status to source - {self.host}:{self.port} - and is no longer suspect"
                )
                await self._logger.filesystem.aio[
                    f"hyperscale.distributed.{self._instance_id}"
                ].debug(
            ...
            self._local_health_multipliers[(source_host, source_port)] = (
                self._reduce_health_multiplier(source_host, source_port)
            )
    ...
    async def receive_forwarded_entries(
        ...
        if self._raft_node_status == NodeState.LEADER and message.entries:
            ...
            self._tasks_queue.append(
                asyncio.create_task(self._submit_logs_to_members(entries))
            )
    ...
    async def receive_failure_notification(
        ...
        try:
            ...
            if (
                not_self
                and self._election_status == ElectionState.READY
                and failed_node not in self.failed_nodes
            ):
                self.failed_nodes.append((host, port, time.monotonic()))
                ...
                self._tasks_queue.append(
                    asyncio.create_task(self.run_election(failed_node=failed_node))
                )
    ...
    async def request_vote(self, host: str, port: int) -> Call[RaftMessage]:
        return RaftMessage(
            host=host,
            port=port,
            source_host=self.host,
    ...
    async def forward_entries_to_leader(
        self, host: str, port: int, entries: List[Entry]
    ) -> Call[RaftMessage]:
        return RaftMessage(
            host=host,
            port=port,
    ...
    async def submit_failure_notification(
        self, host: str, port: int, failed_node: Tuple[str, int]
    ) -> Call[RaftMessage]:
        return RaftMessage(
            host=host,
            port=port,
    ...
    async def _start_suspect_monitor(self):
        ...
        if (
            ...
            self.failed_nodes.append((suspect_host, suspect_port, time.monotonic()))
    ...
    async def push_entries(self, entries: List[Dict[str, Any]]) -> List[RaftMessage]:
        entries.append(
            {
                "key": "logs_update",
                "value": f"Node - {self.host}:{self.port} - submitted log update",
        ...
        if self._raft_node_status == NodeState.LEADER:
            ...
            await self._logger.distributed.aio.info(
                f"Source - {self.host}:{self.port} - pushed - {entries_count} - entries to - {results_count} - members"
            )
            await self._logger.filesystem.aio[
                f"hyperscale.distributed.{self._instance_id}"
            ].info(
            ...
            try:
                ...
                result = await asyncio.wait_for(
                    self.forward_entries_to_leader(
                        current_leader_host, current_leader_port, entries
                    ),
                    timeout=self._calculate_current_timeout(
                        current_leader_host, current_leader_port
                    ),
                )

                await self._logger.distributed.aio.info(
                    f"Source - {self.host}:{self.port} - forwarded - {entries_count} - entries to leader at - {current_leader_host}:{current_leader_port}"
                )
                await self._logger.filesystem.aio[
                    f"hyperscale.distributed.{self._instance_id}"
                ].info(
                ...
                await self._logger.distributed.aio.info(
                    f"Source - {self.host}:{self.port} - encountered error - {str(forward_error)} - out forwarding - {entries_count} - entries to leader at - {current_leader_host}:{current_leader_port}"
                )
                await self._logger.filesystem.aio[
                    f"hyperscale.distributed.{self._instance_id}"
                ].info(
                ...
                return [
                    RaftMessage(
                        ...
                        source_host=self.host,
                        source_port=self.port,
                        elected_leader=(current_leader_host, current_leader_port),
    ...
    def submit_entries(self, entries: List[Dict[str, Any]]):
        self._tasks_queue.append(asyncio.create_task(self.push_entries(entries)))

    def _convert_data_to_entries(self, entries: List[Dict[str, Any]]) -> List[Entry]:
        current_leader_host, current_leader_port = self._term_leaders[-1]
    ...
    def _get_max_instance_id(self):
        ...
        nodes.append((self.host, self.port))
    ...
    async def _cancel_election(self, message: RaftMessage):
        self._election_status = ElectionState.READY
        self._term_number = message.term_number
        ...
        if self._election_task:
            ...
            await self._logger.distributed.aio.info(
                f"Source - {self.host}:{self.port} - election for term - {self._term_number} - was cancelled due to leader reporting for term"
            )
            await self._logger.filesystem.aio[
                f"hyperscale.distributed.{self._instance_id}"
            ].info(
    ...
    async def _update_logs(
        ...
        entries: List[Entry],
        failed_node: Optional[Tuple[str, int]] = None,
    ) -> Union[Tuple[int, RaftMessage], None]:
        shard_id: Union[int, None] = None
        update_response: Union[RaftMessage, None] = None

        await self._logger.distributed.aio.debug(
            f"Running UDP logs update for node - {host}:{port} - for source - {self.host}:{self.port}"
        )
        await self._logger.filesystem.aio[
            f"hyperscale.distributed.{self._instance_id}"
        ].debug(
            ...
            try:
                response = await asyncio.wait_for(
                    self.submit_log_update(
                        host, port, entries, failed_node=failed_node
                    ),
                ...
                source_host, source_port = (
                    update_response.source_host,
                    update_response.source_port,
                )

                not_self = self._check_is_not_self(source_host, source_port)

                if not_self:
                    self._node_statuses[(source_host, source_port)] = (
    ...
    async def notify_of_failed_node(self, failed_node: Tuple[str, int]):
        monitors = [
            address
            for address, status in self._node_statuses.items()
            if status == "healthy" and address != failed_node
        ]

        responses: List[
            Union[Tuple[int, RaftMessage], Exception]
        ] = await asyncio.gather(
            *[
                asyncio.wait_for(
                    self.submit_failure_notification(host, port, failed_node),
                    timeout=self._calculate_current_timeout(host, port),
                )
                for host, port in monitors
            ],
    ...
    async def run_election(self, failed_node: Optional[Tuple[str, int]] = None):
        # Trigger new election
        next_term = self._term_number + 1
        self._raft_node_status = NodeState.CANDIDATE

        await self._logger.distributed.aio.info(
            f"Source - {self.host}:{self.port} - Running election for term - {next_term}"
        )
        await self._logger.filesystem.aio[
            f"hyperscale.distributed.{self._instance_id}"
        ].info(
        ...
        if elected_host == self.host and elected_port == self.port:
            await self._logger.distributed.aio.info(
                f"Source - {self.host}:{self.port} - was elected as leader for term - {next_term}"
            )
            await self._logger.filesystem.aio[
                f"hyperscale.distributed.{self._instance_id}"
            ].info(
            ...
            await asyncio.gather(
                *[
                    asyncio.wait_for(
                        self._update_logs(
                            host, port, latest_logs, failed_node=failed_node
                        ),
                        timeout=self._calculate_current_timeout(host, port),
                    )
            ...
            await self._logger.distributed.aio.info(
                f"Source - {self.host}:{self.port} - failed to receive majority votes and is reverting to a follower for term - {self._term_number}"
            )
            await self._logger.filesystem.aio[
                f"hyperscale.distributed.{self._instance_id}"
            ].info(
    ...
    async def _submit_logs_to_members(self, entries: List[Entry]) -> List[RaftMessage]:
        members: List[Tuple[str, int]] = [
            address
            for address, status in self._node_statuses.items()
            if status == "healthy"
        ...
        results: List[Tuple[int, RaftMessage]] = await asyncio.gather(
            *[
                asyncio.create_task(self._update_logs(host, port, latest_logs))
                for host, port in members
            ]
        )
    ...
    async def _cleanup_pending_raft_tasks(self):
        ...
        while self._running:
            ...
            await self._logger.distributed.aio.debug(
                f"Cleaned up - {pending_count} - for source - {self.host}:{self.port}"
            )
            await self._logger.filesystem.aio[
                f"hyperscale.distributed.{self._instance_id}"
            ].debug(
    ...
    async def leave(self):
        await self._logger.distributed.aio.debug(
            f"Shutdown requested for RAFT source - {self.host}:{self.port}"
        )
        await self._logger.filesystem.aio[
            f"hyperscale.distributed.{self._instance_id}"
        ].debug(f"Shutdown requested for RAFT source - {self.host}:{self.port}")
        ...
        await self._logger.distributed.aio.debug(
            f"Shutdown complete for RAFT source - {self.host}:{self.port}"
        )
        await self._logger.filesystem.aio[
            f"hyperscale.distributed.{self._instance_id}"
        ].debug(f"Shutdown complete for RAFT source - {self.host}:{self.port}")
