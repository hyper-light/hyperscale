...
class ReplicationController(Monitor):
    ...
    async def start(self):
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Starting server for node - {self.host}:{self.port} - with id - {self._instance_id}')
    ...
    async def _run_registration(self):
        ...
        while self._check_all_nodes_registered() is False:
            ...
            if registered_count > last_registered_count:
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - reporting - {registered_count}/{self._initial_expected_nodes} - as fully registered')
    ...
    async def receive_log_update(
        ...
        try:
            ...
            if message.term_number > self._term_number:
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Term number for source - {self.host}:{self.port} - was updated to - {self._term_number} - and leader was updated to - {leader_host}:{leader_port}')
            ...
            if message.failed_node and self._suspect_tasks.get(
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Node - {node_host}:{node_port} - submitted healthy status to source - {self.host}:{self.port} - and is no longer suspect')
            ...
            if self._suspect_tasks.get((
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Node - {source_host}:{source_port} - submitted healthy status to source - {self.host}:{self.port} - and is no longer suspect')
    ...
    async def push_entries(
        ...
        if self._raft_node_status == NodeState.LEADER:
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - pushed - {entries_count} - entries to - {results_count} - members')
            ...
            try:
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - forwarded - {entries_count} - entries to leader at - {current_leader_host}:{current_leader_port}')
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - encountered error - {str(forward_error)} - out forwarding - {entries_count} - entries to leader at - {current_leader_host}:{current_leader_port}')
    ...
    async def _cancel_election(
        ...
        if self._election_task:
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - election for term - {self._term_number} - was cancelled due to leader reporting for term')
    ...
    async def _update_logs(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Running UDP logs update for node - {host}:{port} - for source - {self.host}:{self.port}')
    ...
    async def run_election(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - Running election for term - {next_term}')
        ...
        if elected_host == self.host and elected_port == self.port:
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - was elected as leader for term - {next_term}')
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - failed to receive majority votes and is reverting to a follower for term - {self._term_number}')
    ...
    async def _cleanup_pending_raft_tasks(self):
        ...
        while self._running:
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Cleaned up - {pending_count} - for source - {self.host}:{self.port}')
    ...
    async def leave(self):
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Shutdown requested for RAFT source - {self.host}:{self.port}')
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Shutdown complete for RAFT source - {self.host}:{self.port}')
