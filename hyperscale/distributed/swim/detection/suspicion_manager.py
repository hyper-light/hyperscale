...
class SuspicionManager:
        ...
        async with self._lock:
            ...
            if state and state.incarnation == incarnation:
                if state.add_confirmation(from_node):
                    await self._reschedule_timer(state)
                    return True
            return False
    ...
    def _cleanup_orphaned_unlocked(
        self,
    ) -> tuple[int, list[tuple[tuple[str, int], int]]]:
        """
        ...
        for node, state in list(self.suspicions.items()):
            ...
            has_raw_timer = (
                state._timer_task is not None and not state._timer_task.done()
            )
