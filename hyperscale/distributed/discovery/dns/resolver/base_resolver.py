...
from hyperscale.distributed.discovery.dns.core.record.record_data_types import (
    CNAMERecordData,
    NSRecordData,
)
from hyperscale.distributed.discovery.dns.core.url import URL, InvalidHost, InvalidIP
from hyperscale.distributed.discovery.dns.request.dns_client import DNSClient
from hyperscale.distributed.env import Env, RegistrarEnv, load_env
from hyperscale.distributed.env.time_parser import TimeParser
from hyperscale.distributed.models.dns import DNSEntry, DNSMessage
...
class BaseResolver:
    ...
    def __init__(
        ...
        self.client = DNSClient(host, port, instance_id, env)
    ...
    async def query(
        ...
        try:
            return await asyncio.wait_for(
                self._query(fqdn, record_type, skip_cache),
    ...
    async def request(self, fqdn: str, message: DNSMessage, url: URL) -> DNSMessage:
    ...
    def _add_cache_qtype(
        self, msg: DNSMessage, fqdn: str, record_type: RecordType
    ) -> bool:
        ...
        for rec in self.cache.query(fqdn, record_type):
            if isinstance(rec.data, NSRecordData):
                a_res = list(
                    self.cache.query(rec.data.data, (RecordType.A, RecordType.AAAA))
                )
    ...
    def query_cache(self, msg: DNSMessage, fqdn: str, record_type: RecordType):
        cnames = set()
