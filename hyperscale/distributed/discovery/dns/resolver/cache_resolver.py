...
from hyperscale.distributed.discovery.dns.core.record import Record, RecordType
...
from hyperscale.distributed.models.dns import DNSEntry, DNSMessage, QueryType
...
class CacheResolver:
    ...
    async def _query(
        self, _fqdn: str, _record_type: RecordType
    ...
    async def query_local(
        self, fqdn: str, record_type: RecordType = RecordType.ANY
    ) -> Tuple[DNSMessage, bool]:
        if fqdn.endswith("."):
            fqdn = fqdn[:-1]
    ...
    def query_cache(
        self, fqdn: str, record_type: RecordType
    ) -> Tuple[DNSMessage, bool]:
        if fqdn.endswith("."):
            fqdn = fqdn[:-1]
        ...
        if record_type != RecordType.CNAME:
            has_result = (
                self._add_cache_record_type(msg, fqdn, record_type) or has_result
            )
        ...
        if any(("." + fqdn).endswith(root) for root in self.zone_domains):
            ...
            msg = DNSMessage(**msg.dict(), query_authoritative_answer=1)
