...
class SOARecordData(RecordData):
    def __init__(self, *args):
        super().__init__(RecordType.SOA, data=args)
    ...
    def load(
        ...
        (
            ...
            expire,
            minimum,
        ) = struct.unpack("!LLLLL", data[cursor_position : cursor_position + 20])
    ...
    def dump(self, names: Dict[str, int], offset: int) -> Iterable[bytes]:
        ...
        domain_name = pack_domain_name(self.rname, names, offset + 2 + mname_length)

        record_bytes = struct.pack(
            "!LLLLL", self.serial, self.refresh, self.retry, self.expire, self.minimum
        )

        record_data = [mname, domain_name, record_bytes]
