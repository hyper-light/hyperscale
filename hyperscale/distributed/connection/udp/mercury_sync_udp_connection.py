...
from hyperscale.distributed.snowflake.snowflake_generator import SnowflakeGenerator
...
class MercurySyncUDPConnection:
    ...
    async def send(
        self, event_name: str, data: Any, addr: Tuple[str, int]
    ) -> Tuple[int, Dict[str, Any]]:
        item = pickle.dumps(
            ("request", self.id_generator.generate(), event_name, data),
    ...
    async def send_bytes(
        self, event_name: str, data: bytes, addr: Tuple[str, int]
    ) -> bytes:
        try:
    ...
    async def stream(
        ...
        item = pickle.dumps(
            ("stream", self.id_generator.generate(), event_name, data),
            protocol=pickle.HIGHEST_PROTOCOL,
        )
        ...
        try:
            ...
            for item in self.queue[event_name]:
                (_, shard_id, _, response_data, _, _) = item
    ...
    def read(self, data: bytes, addr: Tuple[str, int]) -> None:
        decrypted = self._encryptor.decrypt(self._decompressor.decompress(data))

        result: Tuple[str, int, float, Any] = pickle.loads(decrypted)

        (message_type, shard_id, event_name, payload) = result
    ...
    async def _reset_connection(self):
        try:
            ...
            await self.connect_async(
                cert_path=self._udp_cert_path, key_path=self._udp_key_path
            )
    ...
    async def _read(
        self, event_name: str, coroutine: Coroutine, addr: Tuple[str, int]
    ...
    async def _read_iterator(
        self, event_name: str, coroutine: AsyncIterable[Message], addr: Tuple[str, int]
    ) -> Coroutine[Any, Any, None]:
