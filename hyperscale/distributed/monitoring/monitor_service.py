...
class Monitor(Controller):
    ...
    async def deregister_node(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Node - {source_host}:{source_port} - submitted request to leave to source - {self.host}:{self.port}')
        ...
        if self._suspect_tasks.get((
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - has cancelled suspicion of node - {source_host}:{source_port} - due to leave request')
        ...
        if node is not None:
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - has accepted request to remove node - {source_host}:{source_port}')
    ...
    async def update_node_status(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Node - {update_node_host}:{update_node_port} - updating status to - {update_status} - for source - {self.host}:{self.port}')
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Node - {update_node_host}:{update_node_port} - updated status to - {update_status} - for source - {self.host}:{self.port}')
    ...
    async def update_as_suspect(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Node - {source_host}:{source_port} - requested a check for suspect source - {self.host}:{self.port}')
        ...
        if self.status == 'healthy':
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Source - {self.host}:{self.port} - received notification it is suspect despite being healthy from node - {source_host}:{source_port}')
    ...
    async def send_indirect_check(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Node - {source_host}:{source_port} - requested an indirect check for node - {target_host}:{target_port} - from source - {self.host}:{self.port}')
        ...
        try:
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Suspect node - {target_host}:{target_port} - responded to an indirect check from source - {self.host}:{self.port} - for node - {source_host}:{source_port}')
            ...
            if self._node_statuses[(target_host, target_port)] != 'failed':
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Suspect node - {target_host}:{target_port} - failed to respond to an indirect check from source - {self.host}:{self.port} - for node - {source_host}:{source_port}')
    ...
    async def update_acknowledged(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Node - {source_host}:{source_port} - acknowledged the indirect check request for node - {target_host}:{target_port} - for source - {self.host}:{self.port}')
    ...
    async def update_node_health(
        ...
        try:
            ...
            if self._suspect_tasks.get((
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Node - {update_node_host}:{update_node_port} - submitted healthy status to source - {self.host}:{self.port} - and is no longer suspect')
    ...
    async def _run_tcp_healthcheck(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Running TCP healthcheck for node - {host}:{port} - for source - {self.host}:{self.port}')
        ...
        if not_self and healthcheck is None and node_status == 'healthy':
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Node - {check_host}:{check_port} - failed to respond over - {self._poll_retries} - retries and is now suspect for source - {self.host}:{self.port}')
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Node - {check_host}:{check_port} - responded on try - {idx}/{self._poll_retries} - for source - {self.host}:{self.port}')
    ...
    async def start(self):
        ...
        await self._logger.filesystem.aio.create_logfile(f'hyperscale.distributed.{self._instance_id}.log')
        self._logger.filesystem.create_filelogger(f'hyperscale.distributed.{self._instance_id}.log')
    ...
    async def register(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Initializing node - {self.host}:{self.port} - with id - {self._instance_id}')
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Connecting to node node - {self.bootstrap_host}:{self.bootstrap_port}')
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Initialized node - {self.host}:{self.port}')
    ...
    async def _register_initial_node(self):
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Connecting to initial node - {self.bootstrap_host}:{self.bootstrap_port}')
    ...
    async def _acknowledge_indirect_probe(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Running UDP healthcheck for node - {host}:{port} - for source - {self.host}:{self.port}')
        ...
        for idx in range(self._poll_retries):
            ...
            try:
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Sending indirect check request to - {target_host}:{target_port} -for node - {host}:{port} - from source - {self.host}:{self.port}')
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Completed indirect check request to - {target_host}:{target_port} -for node - {host}:{port} - from source - {self.host}:{self.port} - on try - {idx}/{self._poll_retries}')
    ...
    async def _run_healthcheck(
        ...
        for idx in range(self._poll_retries):
            ...
            try:
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Node - {host}:{port} - responded on try - {idx}/{self._poll_retries} - for source - {self.host}:{self.port}')
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Node - {host}:{port} - failed for source node - {self.host}:{self.port} - on attempt - {idx}/{self._poll_retries}')
    ...
    async def _start_suspect_monitor(self) -> Tuple[str, int]:
        ...
        if status == 'suspect':
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Node - {suspect_host}:{suspect_port} - marked suspect for source {self.host}:{self.port}')
        ...
        while elapsed < suspicion_timeout and status == 'suspect':
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Source - {self.host}:{self.port} - acknowledged - {indirect_ack_count} - indirect probes and failed to acknowledge - {missing_ack_count} - indirect probes.')
            ...
            if suspect_count < confirmation_members_count:
                ...
                await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Node - {suspect_host}:{suspect_port} - failed to respond for source - {self.host}:{self.port}. Setting next timeout as - {suspicion_timeout}')
        ...
        if self._node_statuses[(suspect_host, suspect_port)] == 'suspect':
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Node - {suspect_host}:{suspect_port} - marked failed for source - {self.host}:{self.port}')
    ...
    async def _request_indirect_probe(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Requesting indirect check for node -  {host}:{port} - for source - {self.host}:{self.port}')
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].info(f'Total of {suspect_count} nodes confirmed node -  {host}:{port} - is suspect for source - {self.host}:{self.port}')
    ...
    async def _push_status_update(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Pushing UDP health update for source - {host}:{port} - to node - {self.host}:{self.port}')
    ...
    async def _push_tcp_status_update(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Pushing TCP health update for source - {host}:{port} - to node - {self.host}:{self.port}')
    ...
    async def _push_suspect_update(
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Pushing TCP health update for source - {host}:{port} - to suspect node - {self.host}:{self.port}')
    ...
    async def cleanup_pending_checks(self):
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Running cleanup for source - {self.host}:{self.port}')
        ...
        while self._running:
            ...
            await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Cleaned up - {pending_checks_count} - for source - {self.host}:{self.port}')
    ...
    async def _shutdown(self):
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Shutdown requested for source - {self.host}:{self.port}')
        ...
        await self._logger.filesystem.aio[f'hyperscale.distributed.{self._instance_id}'].debug(f'Shutdown complete for source - {self.host}:{self.port}')
