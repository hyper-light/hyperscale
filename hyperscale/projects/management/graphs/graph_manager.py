...
class GraphManager:
    ...
    def execute_workflow(self, workflow_actions: List[str]):
        ...
        for workflow_action in workflow_actions:
            self.logger.hyperscale.sync.debug(f'GraphManager: {self.manager_id} - Executing workflow action - {workflow_action}')
            ...
            action.execute()
            self.logger.hyperscale.sync.debug(f'GraphManager: {self.manager_id} - Completed workflow action - {workflow_action}')

    def discover_graph_files(self) -> Dict[str, str]:

        candidate_files = glob.glob(self.config.path + '/**/*.py', recursive=True)
        self.logger.hyperscale.sync.debug(f'GraphManager: {self.manager_id} - Searching for Graph and Plugin files on path - {self.config.path}')

        for candidate_filepath in candidate_files:

            self.logger.hyperscale.sync.debug(f'GraphManager: {self.manager_id} - Analyzing file: {candidate_filepath}')
            ...
            try:
                ...
                if len(graphs) > 0:
                    self.logger.hyperscale.sync.debug(f'GraphManager: {self.manager_id} - Found Graph file at - {candidate_filepath}')               
                    graph_filepath = Path(candidate_filepath)                
                    self.discovered_graphs[graph_filepath.stem] = str(graph_filepath.resolve())

                if len(plugins) > 0:
                    self.logger.hyperscale.sync.debug(f'GraphManager: {self.manager_id} - Found Plugin file at - {candidate_filepath}')     
                    plugin_filepath = Path(candidate_filepath)
                    self.discovered_plugins[plugin_filepath.stem] = str(plugin_filepath.resolve())

            except Exception as e:
                self.logger.hyperscale.sync.error(f'Encountered error loading file at - {str(e)}.')
                pass
