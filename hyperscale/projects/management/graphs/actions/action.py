...
class RepoAction:
    ...
    def _pull_from_remote(self):
        self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Cloning from remote - {self.config.uri} - to path - {self.config.path}')
        self.repo = Repo.clone_from(self.config.uri, self.config.path)
        self.branch = self.repo.create_head(self.config.branch)

        self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} -Setting branch - {self.config.branch}')

        self.remote = self.repo.remote(name=self.config.remote)

        self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} -Setting remote - {self.config.remote}')

        self.git = self.repo.git

    def _setup(self):
        self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Re-initializing repo at - {self.config.path}')
    ...
    def _checkout(self):
        ...
        if self.branch in self.repo.branches and self.branch.name != self.repo.head.name:
            self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Checking out existing branch - {self.config.branch}')
            self.branch.checkout()

        else:
            self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Creating new branch - {self.config.branch}')
            self.branch = self.repo.create_head(self.config.branch)

        self.repo.head.reference = self.branch
        
        self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Setting remote - {self.config.remote} - to track from branch - {self.config.branch}')
        ...
        self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Checkout complete')

    def _update_ignore(self, force_create=False):

        self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Updating .gitignore at {self.config.path}/.gitignore')
        ...
        if os.path.exists(gitignore_path) or force_create:
            self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Creating or updating .gitignore')

            if os.path.exists(gitignore_path):
                self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Reading existing .gitignore to check for already ignored items')
            ...
            with open(gitignore_path, 'a+') as hyperscale_gitignore:
                ...
                self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Adding {len(filter_files)} new files to .gitignore')

                if len(filter_files) > 0:
                    hyperscale_gitignore.writelines(f'\n{filter_files_data}\n')
                    
            self.repo.index.add('.gitignore')
            self.logger.hyperscale.sync.debug(f'GitAction: {self.action_id} - Update of .gitignore at - {self.config.path}/.gitignore - complete')
