...
class Generator:
    def __init__(
        self, generator_types: Dict[str, Any], known_module_paths: Dict[str, str] = {}
    ) -> None:
    ...
    def gather_required_items(self, generator_type: str) -> Dict[str, Any]:
        ...
        modules = {
            ...
            if member_name in local_file_items
        }

        for module_name, module in modules.items():
            try:
                same_source_file = inspect.getfile(module) == inspect.getfile(
                    generator_item
                )
    ...
    def collect_imports(self, generator_type: Optional[str], modules: Dict[str, Any]):
        ...
        for module_name, module in modules.items():
            if inspect.ismodule(module):
                ...
                module_path = self.known_module_paths.get(
                ...
                while run_minimum_path_search:
                    ...
                    if len(minimum_viable_import_path) > 0:
                        ...
                        source_module_members = {
                            ...
                            for source_module_name, source_module in inspect.getmembers(
                                source_module
                            )
                        }
    ...
    def serialize_items(self):
        ...
        self.serialized_locals = [inspect.getsource(local) for local in self.locals]
