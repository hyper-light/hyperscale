...
def create_graph(
    path: str, stages: Optional[str], engine: str, reporter: str, log_level: str
):
    ...
    if stages is None:
        stages_list = ["setup", "execute", "analyze", "submit"]

    else:
        stages_list = stages.split(",")

    generated_stages_count = len(stages_list)
    generated_stages = "".join([f"\n-{stage}" for stage in stages_list])

    logger["console"].sync.info(
        f"Generating - {generated_stages_count} stages:{generated_stages}"
    )
    ...
    for stage in stages_list:
        if stage not in generator.valid_types:
            raise InvalidStageType(
                stage,
                [
                    generator_type_name
                    for generator_type_name, generator_type in generator.generator_types.items()
                    if inspect.isclass(generator_type)
    ...
    with open(path, "w") as generated_test:
        generated_test.write(
            generator.generate_graph(stages_list, engine=engine, reporter=reporter)
        )
