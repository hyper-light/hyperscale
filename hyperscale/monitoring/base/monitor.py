...
class BaseMonitor:
            ...
            for signame in ("SIGINT", "SIGTERM", "SIG_IGN"):
                ...
                signal.signal(
                    ...
                    lambda sig, frame: handle_monitor_stop(
                        running_monitors=self._running_monitors, executor=self._executor
                    ),
                )

        self._sync_background_monitors[monitor_name] = self._executor.submit(
            functools.partial(
                self._monitor_at_interval, monitor_name, interval_sec=interval_sec
            )
        )

    def aggregate_worker_stats(self):
        raise NotImplementedError(
            "Aggregate worker stats method method must be implemented in a non-base Monitor class."
        )
    ...
    def trim_monitor_samples(self, monitor_name: str, trim_length: int):
    ...
    def _monitor_at_interval(
        ...
        try:
            loop.run_until_complete(
                self._update_background_monitor(monitor_name, interval_sec=interval_sec)
            )
    ...
    def stop_background_monitor_sync(self, monitor_name: str):
        self._running_monitors[monitor_name] = False
        self._sync_background_monitors[monitor_name].result()

        if self.active.get(monitor_name):
            self.collected[monitor_name].extend(list(self.active[monitor_name]))

    async def stop_background_monitor(self, monitor_name: str):
        self._running_monitors[monitor_name] = False
