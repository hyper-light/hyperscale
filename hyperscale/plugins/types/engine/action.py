...
class Action(BaseAction[A], Generic[A]):
    __slots__ = ("source", "use_security_context", "security_context", "type")
    ...
    def action_to_serializable(self):
        ...
        serializable = {
            ...
            "metadata": {"user": self.metadata.user, "tags": self.metadata.tags},
            "hooks": self.hooks.to_names(),
            "fields": {},
        }
        for attribute_name, attribute_value in instance_attributes:
            if (
                serializable.get(attribute_name) is None
                and attribute_name not in base_attributes
            ):
