...
from hyperscale.core.engines.types.custom.client import (
    MercuryCustomClient as CustomSession,
)
from hyperscale.plugins.types.common.event import Event
...
class EnginePlugin(Generic[A, R], Plugin):
    ...
    def __init__(self, config: Config) -> None:
        super(EnginePlugin, self).__init__()
    ...
    async def execute(self, *args, **kwargs) -> Awaitable[Result[R]]:
        if self.registered.get(self.next_name) is None:
            action: Action[A] = self.action(self.next_name, *args, **kwargs)
