...
class MercuryUDPClient(BaseEngine[UDPAction, UDPResult]):
    ...
    def __init__(
        ...
        super(MercuryUDPClient, self).__init__()
    ...
    def extend_pool(self, increased_capacity: int):
        ...
        for _ in range(increased_capacity):
            self.pool.connections.append(UDPConnection(self.pool.reset_connections))
        ...
        async with self.sem:
            ...
            try:
                ...
                self.pool.connections.append(
                    UDPConnection(reset_connection=self.pool.reset_connections)
                )
