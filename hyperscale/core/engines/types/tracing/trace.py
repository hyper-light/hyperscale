...
class Trace:
    ...
    def __init__(
        ...
        self.on_connection_reuse_connection: TraceSignal = (
            on_connection_reuse_connection
        )
    ...
    def trace_config_ctx(self, **kwargs: Dict[str, TraceSignal]) -> Trace:
        return Trace(
            tracer=self.tracer,
    ...
    def add_trace(self, trace_signal: TraceSignal) -> None:
        ...
        if trace_name in self.allowed_traces:
            object.__setattr__(self, trace_name, trace_signal)
    ...
    async def on_request_start(
        ...
        if callable(self.url_filter):
            request_url = remove_url_credentials(self.url_filter(request.url))
        ...
        self.token = context_api.attach(trace.set_span_in_context(self.span))
    ...
    async def on_request_end(
        ...
        if self.span.is_recording() and response.status:
            self.span.set_status(Status(http_status_to_status_code(response.status)))

            self.span.set_attribute(SpanAttributes.HTTP_STATUS_CODE, response.status)
    ...
    async def on_request_exception(
        ...
        if self.span.is_recording() and response.error:
            ...
            self.span.record_exception(str(response.error))
