...
from hyperscale.core.engines.types.http2.events.deferred_headers_event import (
    DeferredHeaders,
)
from hyperscale.core.engines.types.http2.events.remote_settings_changed_event import (
    RemoteSettingsChanged,
)
from hyperscale.core.engines.types.http2.events.settings_acknowledged_event import (
    SettingsAcknowledged,
)
from hyperscale.core.engines.types.http2.events.stream_ended_event import StreamEnded
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self.type == 0xA:
            ...
            self.defined_flags = [Flag("END_HEADERS", 0x04)]
            ...
            self.defined_flags = [Flag("ACK", 0x01)]
            ...
            # PUSH PROMISE
            self.defined_flags = [Flag("END_HEADERS", 0x04), Flag("PADDED", 0x08)]
    ...
    def get_events_and_frames(self, stream: Stream, connection):
        if self.type == 0x0:
            ...
            try:
                ...
                conn_increment = conn_manager.process_bytes(flow_controlled_length)
            ...
            deferred_headers = DeferredHeaders(
                stream.encoder, self, connection._h2_state.config.header_encoding
            )
            ...
            if "ACK" in self.flags:
                ...
                if max_header_list_size_change is not None:
                    connection._decoder.max_header_list_size = (
                ...
                if header_table_size_change:
                    ...
                    # respect it.
                    connection._decoder.max_allowed_table_size = (
            ...
            if initial_window_size_change:
                stream.current_outbound_window_size = (
                    connection._guard_increment_window(
                        stream.current_outbound_window_size,
                        initial_window_size_change.new_value
                        - initial_window_size_change.original_value,
                    )
            ...
            if header_table_size_change:
                connection._encoder.header_table_size = (
                    header_table_size_change.new_value
                )
                ...
                try:
                    ...
                    try:
                        connection.outbound_flow_control_window = (
                            connection._guard_increment_window(
                                connection.outbound_flow_control_window, increment
                            )
                        )
                    except StreamError:
