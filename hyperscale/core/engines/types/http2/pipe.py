...
from hyperscale.core.engines.types.common.fast_hpack import Decoder, Encoder
from hyperscale.core.engines.types.common.hpack.table import HeaderTable
from hyperscale.core.engines.types.http2.config import H2Configuration
from hyperscale.core.engines.types.http2.errors.exceptions import (
    StreamClosedError,
    StreamError,
)
from hyperscale.core.engines.types.http2.errors.types import ErrorCodes
from hyperscale.core.engines.types.http2.events.connection_terminated_event import (
    ConnectionTerminated,
)
from hyperscale.core.engines.types.http2.events.data_received_event import DataReceived
from hyperscale.core.engines.types.http2.events.stream_reset import StreamReset
from hyperscale.core.engines.types.http2.events.window_updated_event import (
    WindowUpdated,
)
from hyperscale.core.engines.types.http2.stream import Stream
from hyperscale.core.engines.types.http2.streams.stream_closed_by import StreamClosedBy
from hyperscale.core.engines.types.http2.streams.stream_settings import Settings
from hyperscale.core.engines.types.http2.streams.stream_settings_codes import (
    SettingCodes,
)
from hyperscale.core.engines.types.tracing.trace_session import Trace
...
class HTTP2Pipe:
    ...
    def send_request_headers(self, request: HTTP2Action, stream: Stream):
        ...
        if (
            request.hpack_encoder.header_table.maxsize
            != self._encoder.header_table.maxsize
        ):
    ...
    async def receive_response(
        self, action: HTTP2Action, response: HTTP2Result, stream: Stream, trace: Trace
    ):
        done = False
        while done is False:
            data = b""
            ...
            try:
                data = await asyncio.wait_for(
                    stream.read(), timeout=stream.timeouts.socket_read_timeout
                )
                ...
                try:
                    if frame.type == 0x0:
                        ...
                        try:
                            ...
                            raise Exception(
                                f"Connection - {stream.stream_id} err: {str(e._events[0])}"
                            )
                        ...
                        frames = []
                        raise Exception(
                            f"Connection - {stream.stream_id} err: {str(new_event)}"
                        )
                        ...
                        raise Exception(
                            f"Connection - {stream.stream_id} err: {str(reset_event)}"
                        )
                        ...
                        if "ACK" in frame.flags:
                            ...
                            if initial_window_size_change is not None:
                                ...
                                new_max_window_size = (
                                    stream.inbound.max_window_size + window_delta
                                )
                            ...
                            if max_frame_size_change is not None:
                                stream.max_outbound_frame_size = (
                        ...
                        if frame.stream_id:
                            try:
                                ...
                                try:
                                    ...
                                    raise Exception(
                                        f"Connection - {stream.stream_id} err: {str(event)}"
                                    )
                            except Exception:
                                frames = []
                        else:
