...
from hyperscale.core.engines.types.common.protocols.udp.quic_protocol import (
    FrameType,
    HeadersState,
...
class MercuryHTTP3Client(BaseEngine[Union[A, HTTP3Action], Union[R, HTTP3Result]]):
    ...
    def extend_pool(self, increased_capacity: int):
        ...
        for _ in range(increased_capacity):
            self.pool.connections.append(HTTP3Connection(self.pool.reset_connections))
        ...
        try:
            if self._hosts.get(action.url.hostname) is None:
                socket_configs = await asyncio.wait_for(
                    action.url.lookup(), timeout=self.timeouts.connect_timeout
                )
    ...
    async def execute_prepared_request(
        self, action: HTTP3Action
    ) -> Coroutine[Any, Any, HTTP3Result]:
        ...
        async with self.sem:
            ...
            try:
                ...
                encoder, frame_data = connection.protocol._encoder.encode(
                    stream_id, action.encoded_headers
                )
                ...
                if action.encoded_data:
                    ...
                    connection.protocol._quic.send_stream_data(
                        stream_id,
                        encode_frame(FrameType.DATA, action.encoded_data),
                        True,
                    )
                ...
                if status >= 300 and status < 400:
                    ...
                    for redirect_number in range(action.redirects):
                        ...
                        stream_id = (
                            connection.protocol._quic.get_next_available_stream_id()
                        )
                        ...
                        if action.encoded_data:
                            stream = connection.protocol._get_or_create_stream(
                                stream_id
                            )
                            if stream.headers_send_state != HeadersState.AFTER_HEADERS:
                                raise Exception(
                        ...
                        response_frames: ResponseFrameCollection = (
                        ...
                        for (
                            header_key,
                            header_value,
                        ) in response_frames.headers_frame.headers:
                            headers[header_key] = header_value
                ...
                self.pool.connections.append(
                    HTTP3Connection(reset_connection=self.pool.reset_connections)
                )
