import asyncio
from typing import TypeVar, List, Dict, Any, Generic, Coroutine
            ...
            results: List[Dict[str, Any]] = await asyncio.gather(
                *[
                    before.call(
                        **{
                            name: value
                            for name, value in action.action_args.items()
                            if name in before.params
                        }
                    )
            ...
            for before_event, result in zip(before_batch, results):
                for data in result.values():
                    if isinstance(result, dict):
                        ...
                        action.action_args.update({before_event.shortname: data})
    ...
    async def execute_after(self, action: A, response: R) -> Coroutine[Any, Any, R]:
        ...
        if action.hooks.notify:
            action.action_args.update(
                {
                    name: action_or_task.action
                    for name, action_or_task in action.hooks.listeners.items()
                }
            )
            ...
            for after_event, result in zip(after_batch, results):
                for data in result.values():
                    if isinstance(result, dict):
                        ...
                        action.action_args.update({after_event.shortname: data})
            ...
            for check_event, result in zip(check_batch, results):
                for data in result.values():
                    if isinstance(result, dict):
                        ...
                        action.action_args.update({check_event.shortname: data})
