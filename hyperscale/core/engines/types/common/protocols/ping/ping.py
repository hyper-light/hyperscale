...
class PingConnection:
    ...
    async def receive(
        self, active_socket: socket.socket, id_: bytes, timeout: float
    ) -> None:
        """
    ...
    async def send(
        ...
        header = struct.pack("BbHHh", icmp_type, 0, ping_checksum, id_, 1)

        bytes_in_double = struct.calcsize("d")
        data = (192 - bytes_in_double) * "Q"
        data = struct.pack("d", time.perf_counter()) + data.encode("ascii")
        ...
        header = struct.pack("BbHHh", icmp_type, 0, socket.htons(ping_checksum), id_, 1)
        packet = header + data
        ...
        asyncio.get_event_loop().add_writer(active_socket, callback)
