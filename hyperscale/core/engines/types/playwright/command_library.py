...
class CommandLibrary:
    ...
    async def goto(self, command: PlaywrightCommand):
        await self.page.goto(command.url.location, **command.options.extra)

    async def get_url(self, command: PlaywrightCommand):
        return self.page.url

    async def fill(self, command: PlaywrightCommand):
        await self.page.fill(command.page.selector, command.input.text)

    async def check(self, command: PlaywrightCommand):
        await self.page.check(command.page.selector, **command.options.extra)

    async def click(self, command: PlaywrightCommand):
        await self.page.click(command.page.selector, **command.options.extra)

    async def double_click(self, command: PlaywrightCommand):
        await self.page.dblclick(command.page.selector, **command.options.extra)

    async def submit_event(self, command: PlaywrightCommand):
        await self.page.dispatch_event(
            command.page.selector, command.options.event, **command.options.extra
        )
    ...
    async def evaluate_expression(self, command: PlaywrightCommand):
        await self.page.evaluate(
            command.input.expression, *command.input.args, **command.options.extra
        )
    ...
    async def press_key(self, command: PlaywrightCommand):
        await self.page.press(
            command.page.selector, command.input.key, **command.options.extra
        )
    ...
    async def verify_is_hidden(self, command: PlaywrightCommand):
        return await self.page.is_hidden(command.page.selector, **command.options.extra)
    ...
    async def take_screenshot(self, command: PlaywrightCommand):
        await self.page.screenshot(
            path=f"{command.input.path}.png", **command.options.extra
        )
    ...
    async def select_option(self, command: PlaywrightCommand):
        if command.input.by_label:
            ...
            await self.page.select_option(
                command.page.selector, command.input.option, **command.options.extra
            )

    async def set_checked(self, command: PlaywrightCommand):
        await self.page.set_checked(
            command.page.selector, command.options.is_checked, **command.options.extra
        )
    ...
    async def tap(self, command: PlaywrightCommand):
        await self.page.tap(command.page.selector, **command.options.extra)

    async def get_text_content(self, command: PlaywrightCommand):
        await self.page.text_content(command.page.selector, **command.options.extra)
    ...
    async def input_text(self, command: PlaywrightCommand):
        await self.page.type(
            command.page.selector, command.input.text, **command.options.extra
        )

    async def uncheck(self, command: PlaywrightCommand):
        await self.page.uncheck(command.page.selector, **command.options.extra)

    async def wait_for_event(self, command: PlaywrightCommand):
        await self.page.wait_for_event(command.options.event, **command.options.extra)
    ...
    async def wait_for_url(self, command: PlaywrightCommand):
        await self.page.wait_for_url(command.url.location, **command.options.extra)
    ...
    async def get_attribute(self, command: PlaywrightCommand):
        return await self.page.get_attribute(
            command.page.selector, command.page.attribute
        )
