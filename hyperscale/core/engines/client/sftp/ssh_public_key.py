...
from .types import FilePath, DefTuple, IPNetwork
from .files import read_file, write_file
from .network import ip_network
from .units import parse_time_interval


_Comment = Optional[bytes | str]
...
_PassphraseCallable = Callable[[str], bytes | str]
_PassphraseArg = Optional[Union[_PassphraseCallable, bytes | str]]
...
def _resolve_passphrase(
        ...
        loop: Optional[asyncio.AbstractEventLoop]) -> Optional[bytes | str]:
    """Resolve a passphrase used to encrypt/decrypt SSH private keys"""

    resolved_passphrase: Optional[bytes | str]
...
class SSHOpenSSHCertificate(SSHCertificate):
    ...
    def _encode_force_cmd(force_command: object) -> bytes:
        ...
        return String(cast(bytes | str, force_command))
...
def _decode_openssh_private(
        data: bytes, passphrase: Optional[bytes | str],
        unsafe_skip_rsa_key_validation: Optional[bool]) -> SSHKey:
...
def _decode_der_private(
        key_data: object, passphrase: Optional[bytes | str],
        unsafe_skip_rsa_key_validation: Optional[bool]) -> SSHKey:
...
def import_private_key(
        data: bytes | str, passphrase: Optional[bytes | str] = None,
...
def import_public_key(data: bytes | str) -> SSHKey:
    """Import a public key
...
def import_certificate(data: bytes | str) -> SSHCertificate:
...
def read_private_key(
        filename: FilePath, passphrase: Optional[bytes | str] = None,
        unsafe_skip_rsa_key_validation: Optional[bool] = None) -> SSHKey:
...
def load_keypairs(
        keylist: KeyPairListArg, passphrase: Optional[bytes | str] = None,
...
def load_default_keypairs(passphrase: Optional[bytes | str] = None,
