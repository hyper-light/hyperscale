...
class EventGraph:
    def __init__(self, hooks_by_type: Dict[HookType, Dict[str, Hook]]) -> None:
        ...
        self.event_hooks: List[Hook] = [
            *list(self.hooks_by_type.get(HookType.ACTION, {}).values()),
            *list(self.hooks_by_type.get(HookType.CHANNEL, {}).values()),
            *list(self.hooks_by_type.get(HookType.CHECK, {}).values()),
            *list(self.hooks_by_type.get(HookType.CONDITION, {}).values()),
            *list(self.hooks_by_type.get(HookType.CONTEXT, {}).values()),
            *list(self.hooks_by_type.get(HookType.EVENT, {}).values()),
            *list(self.hooks_by_type.get(HookType.LOAD, {}).values()),
            *list(self.hooks_by_type.get(HookType.METRIC, {}).values()),
            *list(self.hooks_by_type.get(HookType.SAVE, {}).values()),
            *list(self.hooks_by_type.get(HookType.TASK, {}).values()),
        ...
        self.base_stages = list(
            set([node.stage_instance.__class__.__base__.__name__ for node in nodes])
        )
    ...
    def hooks_to_events(self) -> EventGraph:
        for event_hook in self.event_hooks:
            for idx, target_hook_name in enumerate(event_hook.names):
                ...
                if target is None:
                    target: Hook = self.hooks_by_shortname[event_hook.stage].get(
                        target_hook_name
                    )
                ...
                if isinstance(target, Hook):
                    ...
                    self.hooks_graph.update(
                        nodes=[(target.event_name, {"hook": target})]
                    )

                self.events[event.event_name] = event

                self.hooks_graph.update(nodes=[(event.event_name, {"hook": event})])

            if self.events.get(event_hook.name) is None:
                event = get_event(event_hook, event_hook)

                self.events[event_hook.name] = event
                self.hooks_graph.update(nodes=[(event_hook.name, {"hook": event})])
    ...
    def assemble_graph(self) -> EventGraph:
        for event_hook in self.event_hooks:
            for target_hook_name in event_hook.names:
                ...
                if target is None:
                    target_hook: Hook = self.hooks_by_shortname[event_hook.stage].get(
                        target_hook_name
                    )
    ...
    def apply_graph_to_events(self) -> None:
        for event in self.events.values():
            event.execution_path = [
                edge for edge in networkx.bfs_layers(self.hooks_graph, event.event_name)
            ]
            event.previous_map = [
                node for node in self.hooks_graph.predecessors(event.event_name)
            ]
            event.next_map = [
                node for node in self.hooks_graph.successors(event.event_name)
            ]

            for path_layer in event.execution_path:
                event.events.update(
                    {
                        event_name: self.events.get(event_name)
                        for event_name in path_layer
                    }
                )

            hook_names = [
                hook.name for hook in event.source.stage_instance.hooks[event.hook_type]
            ]
            ...
            if len(event.previous_map) < 1 and event.event_name in hook_names:
                ...
                event.source.stage_instance.dispatcher.initial_events[
                    event.source.stage
                ].append(event)
