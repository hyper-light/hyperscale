...
class TransformHook(Hook):
    def __init__(
        ...
        *names: Optional[Tuple[str, ...]],
        ...
        super().__init__(
            name, shortname, call, order=order, skip=skip, hook_type=HookType.TRANSFORM
        )
    ...
    async def call(self, **kwargs):
        ...
        for name, arg in kwargs.items():
            if isinstance(arg, (list, tuple)):
                batchable_args.extend([{**kwargs, name: item} for item in arg])
        ...
        if len(batchable_args) > 0:
            result = await asyncio.wait_for(
                asyncio.gather(
                    *[
                        ...
                        for call_kwargs in batchable_args
                        if (await self._execute_call(**call_kwargs) is True)
                    ]
                ),
            ...
            return {**kwargs, **dict(aggregated_transformm)}
            ...
            return {**kwargs, self.shortname: result}
