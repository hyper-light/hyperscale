...
class MetricHook(Hook):
    def __init__(
        ...
        super().__init__(
            name, shortname, call, order=order, skip=skip, hook_type=HookType.METRIC
        )
    ...
    async def call(self, **kwargs):
        ...
        for signame in ("SIGINT", "SIGTERM", "SIG_IGN"):
            self.loop.add_signal_handler(
                ...
                lambda signame=signame: handle_loop_stop(signame, self.executor),
            )
        ...
        return {**kwargs, **metric_results}

    async def _calculate_stage_custom_metric(
        self, hook_kwargs: Dict[str, Any], set_name: Optional[str] = None
        ...
        return {metric_fullname: custom_metric}

    def _generate_deserialized_results(
        self, results: RawResultsSet
