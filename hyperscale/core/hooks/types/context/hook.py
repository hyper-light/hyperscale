...
class ContextHook(Hook):
    def __init__(
        ...
        *names: Optional[Tuple[str, ...]],
        ...
        super().__init__(name, shortname, call, hook_type=HookType.CONTEXT)
    ...
    async def call(self, **kwargs) -> None:
        ...
        if "results" in list(self.params.keys()):
            ...
            for stage_results in context_value.values():
                results.extend(stage_results.results)

            context_value["results"] = results

        context_result = await self._call(
            **{name: value for name, value in hook_args.items() if name in self.params}
        )
        ...
        if isinstance(context_result, dict):
            ...
            return {**kwargs, **context_result}

        self.context[self.name] = context_result
        return {**kwargs, self.shortname: context_result}
