...
from hyperscale.core.personas.streaming.stream import Stream
from hyperscale.core.personas.types.default_persona.default_persona import (
    DefaultPersona,
    cancel_pending,
)
            ...
            if idx % self._hooks[action_idx].session.pool.size == 0:
                if self.stream.completed_count > 0:
                    if self.stream.completed_count < self.batch.size:
                        increase_percentage = (
                            self.batch.size - self.stream.completed_count
                        ) / self.batch.size
                        increase_amount = math.ceil(
                            increase_percentage * self.stream.last_batch_size
                        )

                        self.stream.last_completed = self.stream.completed_count
                        self.stream.last_batch_size = (
                            self.stream.last_batch_size + increase_amount
                        )
                        ...
                        decrease_percentage = (
                            self.stream.completed_count - self.batch.size
                        ) / self.stream.completed_count
                        decrease_amount = math.ceil(
                            decrease_percentage * self.stream.last_batch_size
                        )

                        self.stream.last_completed = self.stream.completed_count
                        self.stream.last_batch_size = (
                            self.stream.last_batch_size - decrease_amount
                        )
