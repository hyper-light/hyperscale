...
class BaseDistribution:
    ...
    def generate_distribution(self, batch_size: int) -> List[float]:
        return [
            math.ceil(step_batch_size)
            for step_batch_size in self._generate_distribution(scale_factor=batch_size)
        ]
    ...
    def _generate_distribution(self, scale_factor: Optional[int] = None):
        ...
        distribution_sample = self._frozen_distribution.rvs(distribution_size) * normal(
            scale=self._noise_scale, size=distribution_size
        )

        generated_walk = self._generate_random_walk(distribution_sample)

        smoothed_walk = self._smooth_generated_walk(generated_walk, step_size)

        if scale_factor:
            smoothed_walk *= scale_factor

        return self._apply_averaging(smoothed_walk, step_size)
    ...
    def _get_distribution_and_step_size(self) -> Tuple[int, int]:
        if self.size <= 10:
            ...
            distribution_size = self.size * (1 + int(math.sqrt(self.size) / self.size))

            step_size = 1 + int(math.sqrt(self.size) / self.size)
    ...
    def _smooth_generated_walk(
        self, generated_walk: List[float], smoothing_window_size: int
    ) -> List[float]:
        return numpy.convolve(
            generated_walk, numpy.ones((smoothing_window_size,)) / smoothing_window_size
        )[(smoothing_window_size - 1) :]

    def _apply_averaging(self, scaled_walk: List[float], step_size: int) -> List[float]:
        averaged_data = []

        for idx in range(0, len(scaled_walk), step_size):
            averaged_data.append(numpy.mean(scaled_walk[idx : idx + self.size]))
