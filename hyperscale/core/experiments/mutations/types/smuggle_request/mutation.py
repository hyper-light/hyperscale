...
class SmuggleRequest(Mutation):
    def __init__(
        ...
        *targets: Tuple[str, ...],
        ...
        super().__init__(name, chance, MutationType.SMUGGLE_REQUEST, *targets)

        validated_mutation = SmuggleRequestValidator(
            request_size=request_size, smuggled_request=smuggled_request
        )
    ...
    async def mutate(self, action: Request = None) -> Request:
        ...
        if self.smuggled_request is None:
            encoded_smuggled_request: str = "".join(
                random.choices(self._character_pool, k=self.request_size)
            )
