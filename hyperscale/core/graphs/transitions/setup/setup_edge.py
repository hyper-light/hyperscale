...
class SetupEdge(BaseEdge[Setup]):
    def __init__(self, source: Setup, destination: BaseEdge[Stage]) -> None:
        super(SetupEdge, self).__init__(source, destination)

        self.valid_states = [StageStates.INITIALIZED, StageStates.VALIDATED]
    ...
    def _update(self, destination: Stage):
        ...
        if self.skip_stage is False:
            ...
            setup_stage_experiment_config: Dict[str, Union[str, int, List[float]]] = (
                self.edge_data.get("setup_stage_experiment_config", {})
            )
            execute_stage_setup_hooks = []
            setup_execute_stage: Execute = ready_stages.get(self.source.name)

            if setup_execute_stage:
                execute_stage_setup_hooks = setup_execute_stage.context[
                    "execute_stage_setup_hooks"
                ]
            ...
            session_stage_monitors.update(self.edge_data["setup_stage_monitors"])
    ...
    def split(self, edges: List[SetupEdge]) -> None:
        ...
        for event in setup_stage_copy.dispatcher.events_by_name.values():
            ...
            if event.source.shortname in user_hooks[setup_stage_copy.name]:
                hook_call = user_hooks[setup_stage_copy.name].get(
                    event.source.shortname
                )

                hook_call = hook_call.__get__(
                    setup_stage_copy, setup_stage_copy.__class__
                )
    ...
    def get_setup_candidates(self) -> Dict[str, Execute]:
        ...
        for stage_name in path_lengths.keys():
            ...
            if stage_name in execute_candidates:
                if len(following_setup_stage_distances) > 0 and stage_distance <= min(
                    following_setup_stage_distances
                ):
                    selected_execute_candidates[stage_name] = execute_candidates.get(
                        stage_name
                    )
        ...
        for candidate in selected_execute_candidates.values():
            actions = [
                event
                for event in candidate.dispatcher.actions_and_tasks.values()
    ...
    def get_optimize_candidates(self) -> Dict[str, Execute]:
        optimize_stages: Dict[str, Execute] = self.stages_by_type.get(
            StageTypes.OPTIMIZE, {}
        )
        setup_stages: Dict[str, Setup] = self.stages_by_type.get(StageTypes.SETUP)
        path_lengths: Dict[str, int] = self.path_lengths.get(self.source.name)
        ...
        following_setup_stage_distances = [
            path_length
            for stage_name, path_length in path_lengths.items()
            if stage_name in setup_stages or stage_name in optimize_stages
        ]
        ...
        for stage_name in path_lengths.keys():
            ...
            if stage_name in optimize_candidates:
                if len(following_setup_stage_distances) > 0 and stage_distance <= min(
                    ...
                    selected_optimize_candidates[stage_name] = optimize_candidates.get(
                        stage_name
                    )
