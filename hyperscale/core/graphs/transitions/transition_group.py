...
class TransitionGroup:
    def __init__(self) -> None:
        ...
        self.destination_groups: Dict[Tuple[str, StageTypes], List[Transition]] = (
            defaultdict(list)
        )
        self.targets: Dict[str, Stage] = {}
        self.transitions_by_edge: Dict[
            Tuple[
                str,
                str,
            ],
    ...
    def add_transition(self, transition: Transition):
        ...
        self.destination_groups[
            (transition.from_stage.name, transition.to_stage.stage_type)
        ].append(transition)
        self.transitions_by_type[transition.from_stage.stage_type].append(transition)
        self.transitions_by_edge[
            (transition.edge.source.name, transition.edge.destination.name)
        ] = transition
    ...
    def sort_and_map_transitions(self):
        for edge_key, transition in self.transitions_by_edge.items():
            transition.edges = [
                group_transition.edge
                for group_transition in self.transitions_by_edge.values()
            ]

            destinations = self.adjacency_list[transition.edge.source.name]
            transition.destinations = [transition.edge.destination.name]
            ...
            if len(destinations) > 1:
                ...
                transition.destinations = [
                    destination_transition.edge.destination.name
                    for destination_transition in destinations
                ]
        ...
        for group in batched_transitions:
            ...
            for transition_config in group:
                ...
                transition = self.transitions_by_edge.get((source, destination))

                transition_priority: StagePriority = (
            ...
            if len(group_priorities) > 0:
                ...
                prioritized_groups.append((group_priority, group))
        ...
        self._batched_transitions = [group for _, group in sorted_batches]
    ...
    async def execute_group(self):
        ...
        for group in self._batched_transitions:
            sorted_group = sorted(
                group, key=lambda transition: transition[3], reverse=True
            )

            group_results = await asyncio.gather(
                *[
                    asyncio.create_task(self._execute_transition(transition_config))
    ...
    async def _execute_transition(
        self, transition_config: Tuple[str, str, str, int]
    ) -> Any:
        source, destination, _, workers = transition_config
