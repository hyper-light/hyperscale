...
from hyperscale.core.graphs.stages.optimize.optimize import Optimize
...
class OptimizeEdge(BaseEdge[Optimize]):
    def __init__(self, source: Optimize, destination: Stage) -> None:
        super(OptimizeEdge, self).__init__(source, destination)
    ...
    async def transition(self):
        try:
            ...
            self.edge_data["optimize_stage_candidates"] = (
            ...
            if len(selected_optimization_candidates) > 0:
                self.source.generation_optimization_candidates = len(
                    selected_optimization_candidates
                )
    ...
    def _update(self, destination: Stage):
        ...
        if self.skip_stage is False:
            ...
            optimized_config: Stage = self.edge_data[
                "optimize_stage_optimized_configs"
            ].get(destination.name)
            optimzied_hooks: Stage = self.edge_data[
                "optimize_stage_optimized_hooks"
            ].get(destination.name)
            stage_setup_by: str = self.edge_data["execute_stage_setup_by"].get(
                destination.name
            )
            ...
            session_stage_monitors.update(self.edge_data["optimize_stage_monitors"])
            ...
            self.next_history[(self.source.name, destination.name)].update(
                {
                    ...
                    "optimize_stage_optimized_params": self.edge_data[
                        "optimize_stage_optimized_params"
                    ],
                    "setup_stage_candidates": list(
                        self.edge_data["optimize_stage_candidates"].keys()
                    ),
    ...
    def split(self, edges: List[OptimizeEdge]) -> None:
        ...
        optimize_stage_copy: Optimize = type(
            self.source.name, (Optimize,), self.source.__dict__
        )()

        for (
            copied_attribute_name,
            copied_attribute_value,
        ) in optimize_stage_config.items():
            if inspect.ismethod(copied_attribute_value) is False:
                setattr(
                    optimize_stage_copy, copied_attribute_name, copied_attribute_value
                )
        ...
        for event in optimize_stage_copy.dispatcher.events_by_name.values():
            ...
            if event.source.shortname in user_hooks[optimize_stage_copy.name]:
                hook_call = user_hooks[optimize_stage_copy.name].get(
                    event.source.shortname
                )

                hook_call = hook_call.__get__(
                    optimize_stage_copy, optimize_stage_copy.__class__
                )
                ...
                event.source._call = getattr(
                    optimize_stage_copy, event.source.shortname
                )
    ...
    def generate_optimization_candidates(self) -> Dict[str, Stage]:
        ...
        for stage_name, stage in execute_stages.items():
            if stage_name in all_paths:
                stage.context["execute_stage_setup_config"] = self.edge_data[
                    "execute_stage_setup_config"
                ]
                stage.context["execute_stage_setup_by"] = self.edge_data[
                    "execute_stage_setup_by"
                ]
        ...
        following_optimize_stage_distances = [
            ...
            if stage_name in optimize_stages
        ]
        ...
        for stage_name in path_lengths.keys():
            ...
            if stage_name in optimization_candidates:
                if len(
                    following_optimize_stage_distances
                ) > 0 and stage_distance <= min(following_optimize_stage_distances):
                    selected_optimization_candidates[stage_name] = (
                        optimization_candidates.get(stage_name)
                    )
        ...
        return {
            ...
            for candidate, candidate_config in selected_optimization_candidates.items()
            if candidate in self.edge_data["setup_stage_configs"]
        }
