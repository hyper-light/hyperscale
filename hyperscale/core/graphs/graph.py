...
class Graph:
    ...
    def __init__(
        ...
        self.logger.hyperscale.sync.debug(f'{self.metadata_string} - Changed status to - {GraphStatus.INITIALIZING.name} - from - {GraphStatus.IDLE.name}')
        self.logger.filesystem.sync['hyperscale.core'].info(f'{self.metadata_string} - Changed status to - {GraphStatus.INITIALIZING.name} - {GraphStatus.IDLE.name}')

        self.transitions_graph = []
        self._transitions: List[TransitionGroup] = []
        self._results = None

        self.logger.hyperscale.sync.debug(f'{self.metadata_string} - Found - {len(stages)} - stages')
        self.logger.filesystem.sync['hyperscale.core'].debug(f'{self.metadata_string} - Found - {len(stages)} - stages')
        ...
        for stage in stages:
            ...
            self.logger.hyperscale.sync.debug(f'{self.metadata_string} - Adding dependencies for stage - {stage.__name__}')
            self.logger.filesystem.sync['hyperscale.core'].debug(f'{self.metadata_string} - Adding dependencies for stage - {stage.__name__}')

            for dependency in stage.dependencies:
                if self.graph.nodes.get(dependency.__name__):
                    
                    self.logger.hyperscale.sync.debug(f'{self.metadata_string} - Adding edge from stage - {dependency.__name__} - to stage - {stage.__name__}')
                    self.logger.filesystem.sync['hyperscale.core'].debug(f'{self.metadata_string} - Adding edge from stage - {dependency.__name__} - to stage - {stage.__name__}.')
    ...
    def assemble(self):
        ...
        self.logger.hyperscale.sync.debug(f'{self.metadata_string} - Changed status to - {GraphStatus.ASSEMBLING.name} - from - {GraphStatus.INITIALIZING.name}')
        self.logger.filesystem.sync['hyperscale.core'].info(f'{self.metadata_string} - Changed status to - {GraphStatus.ASSEMBLING.name} - from - {GraphStatus.INITIALIZING.name}')
        ...
        self.logger.hyperscale.sync.debug(f'{self.metadata_string} - Prepending {StageTypes.IDLE.name} stage')
        self.logger.filesystem.sync['hyperscale.core'].debug(f'{self.metadata_string} - Prepending {StageTypes.IDLE.name} stage')
        ...
        if len(self.instances.get(StageTypes.ANALYZE)) < 1:
            self.logger.hyperscale.sync.debug(f'{self.metadata_string} - Appending {StageTypes.ANALYZE.name} stage')
            self.logger.filesystem.sync['hyperscale.core'].debug(f'{self.metadata_string} - Appending {StageTypes.ANALYZE.name} stage')
        ...
        if len(self.instances.get(StageTypes.SUBMIT)) < 1:
            self.logger.hyperscale.sync.debug(f'{self.metadata_string} - Appending {StageTypes.SUBMIT.name} stage')
            self.logger.filesystem.sync['hyperscale.core'].debug(f'{self.metadata_string} - Appending {StageTypes.SUBMIT.name} stage')
        ...
        self.logger.hyperscale.sync.debug(f'{self.metadata_string} - Appending {StageTypes.COMPLETE.name} stage')
        self.logger.filesystem.sync['hyperscale.core'].debug(f'{self.metadata_string} - Appending {StageTypes.COMPLETE.name} stage')

        self._append_stage(StageTypes.COMPLETE)

        self.logger.hyperscale.sync.debug(f'{self.metadata_string} - Generating graph stages and transitions')
        self.logger.filesystem.sync['hyperscale.core'].debug(f'{self.metadata_string} - Generating graph stages and transitions')
        ...
        self.logger.filesystem.sync['hyperscale.core'].debug(f'{self.metadata_string} - Assembly complete')
    ...
    async def run(self) -> GraphResults:
        ...
        await self.logger.hyperscale.aio.debug(f'{self.metadata_string} - Changed status to - {GraphStatus.RUNNING.name} - from - {GraphStatus.ASSEMBLING.name}')
        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Changed status to - {GraphStatus.RUNNING.name} - from - {GraphStatus.ASSEMBLING.name}')
            ...
            async with self.logger.spinner as status_spinner:
                ...
                if is_idle_transition is False:
                    ...
                    for transition in transition_group:
                        ...
                        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Executing stage Transition - {transition.transition_id} - from stage - {transition.from_stage.name} - to stage - {transition.to_stage.name}')
                        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Executing stage - {transition.from_stage.name}:{transition.from_stage.stage_id}')
                ...
                for transition in transition_group:
                    ...
                    if error:
                        ...
                        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Changed status to - {GraphStatus.FAILED.name} - from - {GraphStatus.RUNNING.name}')
                        ...
                        await self.logger.filesystem.aio['hyperscale.core'].error(f'{self.metadata_string} - Encountered error executing stage - {source_stage.name}:{source_stage.stage_id}')
                ...
                if is_idle_transition is False:
                    ...
                    await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Completed -  {completed_transitions_count} - transitions')
        ...
        if self.status == GraphStatus.RUNNING:
            ...
            await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Changed status to - {GraphStatus.COMPLETE.name} - from - {GraphStatus.RUNNING.name}')
