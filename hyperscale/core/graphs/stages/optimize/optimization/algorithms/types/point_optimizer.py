...
class PointOptimizer(BaseAlgorithm):
    def __init__(
        ...
        super().__init__(config, distribution_idx=distribution_idx)
    ...
    async def optimize(self, func):
        ...
        for param_name in self.optimize_params:
            for _ in range(self.max_iter):
                error, target_value = await func(list(self.next_params.values()))

                if error:
                    self._compute_next_params(error, target_value, param_name)
    ...
    def _compute_next_params(
        ...
        if error > 0:
            ...
            noise_distribution = normal(scale=self._noise_scale, size=1)

            noise_value = noise_distribution[0]

            adjustment_ratio = abs(round(adjustment_ratio + noise_value, 2))
        ...
        minimum_boundary, maximum_boundary = self._boundaries_by_param_name[
        ...
        if completed > 0:
            ...
            self.error_history[optimizing_param_name].append(absolute_error)

            self.error_batch_size_map[absolute_error].append(last_param_value)
            self.param_errors[optimizing_param_name][last_param_value].append(
                absolute_error
            )
