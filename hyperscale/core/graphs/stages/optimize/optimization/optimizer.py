...
class Optimizer:
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.algorithm: BaseAlgorithm = get_algorithm(
            self.algorithm_type, {**config, "stage_config": self.stage_config}
        )
    ...
    def optimize(self) -> Dict[str, Union[int, float]]:
        ...
        for optimized_param_name, optimized_value in optimized_params.items():
            ...
            self.logger.filesystem.sync["hyperscale.optimize"].info(
                f"{self.metadata_string} - Optimization - {param_log_name} - {optimized_value}"
            )
    ...
    async def _optimize(self, xargs: List[Union[int, float]]) -> float:
        if (
            self._current_iter < self.algorithm.max_iter
            ...
            await self.logger.filesystem.aio["hyperscale.optimize"].debug(
                f"{self.metadata_string} - Optimizer iteration - {self._current_iter} - Inverted APS score- {elapsed/completed_count}"
            )
