...
class Optimizer:
    ...
    def optimize(self) -> Dict[str, Union[int, float]]:
        ...
        self.logger.filesystem.sync['hyperscale.optimize'].info(f'{self.metadata_string} - Starting optimization')
        self.logger.filesystem.sync['hyperscale.optimize'].info(f'{self.metadata_string} - Optimization config: Time Limit - {self._optimization_time_limit}')
        self.logger.filesystem.sync['hyperscale.optimize'].info(f'{self.metadata_string} - Optimization config: Algorithm - {self.algorithm_type}')
        self.logger.filesystem.sync['hyperscale.optimize'].info(f'{self.metadata_string} - Optimization config: Batch Time - {self.algorithm.batch_time}')
        self.logger.filesystem.sync['hyperscale.optimize'].info(f'{self.metadata_string} - Optimization config: Max Iter - {self.algorithm.max_iter}')
        ...
        self.logger.filesystem.sync['hyperscale.optimize'].info(f'{self.metadata_string} - Optimization took - {round(self.total_optimization_time, 2)} - seconds')
        self.logger.filesystem.sync['hyperscale.optimize'].info(f'{self.metadata_string} - Optimization - max actions per second - {self._max_aps}')

        for optimized_param_name, optimized_value in optimized_params.items():
            param_log_name = optimized_param_name.replace('_', ' ')
            self.logger.filesystem.sync['hyperscale.optimize'].info(f'{self.metadata_string} - Optimization - {param_log_name} - {optimized_value}')
    ...
    async def _optimize(self, xargs: List[Union[int, float]]) -> float:
        ...
        if self._current_iter < self.algorithm.max_iter and self.elapsed < self.algorithm.time_limit:
            ...
            await self.logger.filesystem.aio['hyperscale.optimize'].debug(f'{self.metadata_string} - Optimizer iteration - {self._current_iter}')

            await self.logger.filesystem.aio['hyperscale.optimize'].debug(f'{self.metadata_string} - Optimizer iteration - {self._current_iter} - Batch Size - {persona.batch.size}')
            await self.logger.filesystem.aio['hyperscale.optimize'].debug(f'{self.metadata_string} - Optimizer iteration - {self._current_iter} - Batch Interval - {persona.batch.interval}')
            await self.logger.filesystem.aio['hyperscale.optimize'].debug(f'{self.metadata_string} - Optimizer iteration - {self._current_iter} - Batch Gradient - {persona.batch.gradient}')
            ...
            await self.logger.filesystem.aio['hyperscale.optimize'].debug(f'{self.metadata_string} - Optimizer iteration - {self._current_iter} - took - {round(elapsed, 2)} - seconds')
            ...
            await self.logger.filesystem.aio['hyperscale.optimize'].debug(f'{self.metadata_string} - Optimizer iteration - {self._current_iter} - Inverted APS score- {elapsed/completed_count}')
