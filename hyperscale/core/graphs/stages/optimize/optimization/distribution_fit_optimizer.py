...
class DistributionFitOptimizer(Optimizer):
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        for distribution_idx in range(len(self.distribution)):
            self.algorithms.append(
                PointOptimizer(
                    {**config, "stage_config": self.stage_config},
    ...
    async def _optimize_async(self) -> Dict[str, Union[int, float]]:
        ...
        self.logger.filesystem.sync["hyperscale.optimize"].info(
            f"{self.metadata_string} - Starting optimization"
        )
        self.logger.filesystem.sync["hyperscale.optimize"].info(
            f"{self.metadata_string} - Optimization config: Time Limit - {self._optimization_time_limit}"
        )
        self.start = 0
        ...
        for distribution_value, algorithm in zip(self.distribution, self.algorithms):
            ...
            self.algorithm.batch_time = (
                self.stage_config.total_time / self.distribution_intervals
            )

            self.logger.filesystem.sync["hyperscale.optimize"].info(
                f"{self.metadata_string} - Optimization config: Algorithm - {self.algorithm_type}"
            )
            self.logger.filesystem.sync["hyperscale.optimize"].info(
                f"{self.metadata_string} - Optimization config: Batch Time - {self.algorithm.batch_time}"
            )
            self.logger.filesystem.sync["hyperscale.optimize"].info(
                f"{self.metadata_string} - Optimization config: Max Iter - {self.algorithm.max_iter}"
            )
            ...
            ] = await self.algorithm.optimize(self._optimize_iteration)

            optimization_results = results.get("batch_size")
            optimized_distribution_value = optimization_results.get(
                "minimized_distribution_value"
            )
        ...
        self.logger.filesystem.sync["hyperscale.optimize"].info(
            f"{self.metadata_string} - Optimization took - {round(self.total_optimization_time, 2)} - seconds"
        )
        self.logger.filesystem.sync["hyperscale.optimize"].info(
            f"{self.metadata_string} - Optimization - max actions per second - {self._max_aps}"
        )
    ...
    async def _optimize_iteration(
        self, xargs: List[Union[int, float]]
        ...
        await self.logger.filesystem.aio["hyperscale.optimize"].debug(
            f"{self.metadata_string} - Optimizer iteration - {self._current_iter}"
        )

        await self.logger.filesystem.aio["hyperscale.optimize"].debug(
            f"{self.metadata_string} - Optimizer iteration - {self._current_iter} - Batch Size - {persona.batch.size}"
        )
        await self.logger.filesystem.aio["hyperscale.optimize"].debug(
            f"{self.metadata_string} - Optimizer iteration - {self._current_iter} - Batch Interval - {persona.batch.interval}"
        )
        await self.logger.filesystem.aio["hyperscale.optimize"].debug(
            f"{self.metadata_string} - Optimizer iteration - {self._current_iter} - Batch Gradient - {persona.batch.gradient}"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.optimize"].debug(
            f"{self.metadata_string} - Optimizer iteration - {self._current_iter} - took - {round(elapsed, 2)} - seconds"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.optimize"].debug(
            f"{self.metadata_string} - Optimizer iteration - {self._current_iter} - Target error- {error}"
        )
