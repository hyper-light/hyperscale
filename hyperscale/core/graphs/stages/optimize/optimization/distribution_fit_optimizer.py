...
from hyperscale.core.experiments.variant import Variant
...
class DistributionFitOptimizer(Optimizer):
    ...
    async def _optimize_iteration(self, xargs: List[Union[int, float]]) -> Tuple[Union[None, float], int]:
        ...
        await self.logger.filesystem.aio['hyperscale.optimize'].debug(f'{self.metadata_string} - Optimizer iteration - {self._current_iter} - Target error- {error}')
