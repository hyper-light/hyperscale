...
from hyperscale.core.hooks.types.condition.decorator import condition
from hyperscale.core.hooks.types.context.decorator import context
from hyperscale.core.hooks.types.event.decorator import event
...
from hyperscale.plugins.types.extension.types import ExtensionType
...
def handle_loop_stop(signame, executor: ThreadPoolExecutor):
    try:
...
class Optimize(Stage):
    ...
    algorithm: Literal["shg", "dual-annealing", "diff-evolution"] = "shg"
    time_limit = "1m"
    optimize_params: List[Parameter] = [
        Parameter("batch_size", minimum=0.5, maximum=2, feed_forward=True)
    ]
    ...
    async def collect_optimization_stages(
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Optimizing stages {stage_names} using {self.algorithm} algorithm"
        )
        ...
        optimize_stages = [
            (stage.name, stage) for stage in optimize_stage_candidates.values()
        ]
        ...
        batched_stages: BatchedOptimzationCandidates = list(
            self.executor.partion_stage_batches(optimize_stages)
        )
        ...
        return {
            ...
            "optimize_stage_monitors": {"cpu": cpu_monitor, "memory": memory_monitor},
        }
    ...
    async def get_stage_plugins(self):
        ...
        for plugin_name, plugin in optimize_plugins.items():
            registered_algorithms[plugin_name] = lambda plugin_config: plugin(
                plugin_config
            )
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Loaded Optimizer plugin - {plugin_name}"
            )
    ...
    async def collect_loaded_actions(
        ...
        return {"optimize_stage_loaded_actions": loaded_actions}

    @condition("collect_optimization_stages", "get_stage_plugins")
    async def check_has_multiple_workers(self):
        return {"optimize_stage_has_multiple_workers": self.total_pool_cpus > 1}
    ...
    async def create_optimization_configs(
        ...
        if optimize_stage_has_multiple_workers:
            ...
            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Batching optimization for - {optimize_stage_stages_count} stages"
            )
            ...
            for (
            ...
            ) in optimize_stage_batched_stages:
                configs = []
                selected_stage_config = setup_stage_configs.get(stage.name)

                batch_size = int(
                    selected_stage_config.batch_size / assigned_workers_count
                )
                    ...
                    configs.append(
                        {
                            ...
                            "enable_unstable_features": active_flags[
                            ...
                            "execute_setup_stage_name": stage.context[
                ...
                configs[assigned_workers_count - 1]["execute_stage_batch_size"] += (
                    batch_size % assigned_workers_count
                )

                configs = [dill.dumps(config) for config in configs]

                batched_configs.append((stage_name, assigned_workers_count, configs))

                await self.logger.filesystem.aio["hyperscale.core"].debug(
                    f"{self.metadata_string} - Provisioned - {assigned_workers_count} - workers for stage - {stage_name}"
                )

            return {"optimize_stage_batched_configs": batched_configs}
    ...
    async def execute_batched_optimization(
        ...
        if optimize_stage_has_multiple_workers:
            ...
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Starting optimizaiton for - {optimize_stage_stages_count} - stages"
            )

            results = await self.executor.execute_batches(
                optimize_stage_batched_configs, optimize_stage
            )

            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Completed optimizaiton for - {optimize_stage_stages_count} - stages"
            )
            ...
            return {"optimize_stage_results": optimization_results}
    ...
    async def setup_single_worker_optimization(
        ...
        if optimize_stage_has_multiple_workers is False:
            ...
            for stage_name, persona_config in setup_stage_configs.items():
                ...
                for hook in action_and_task_hooks:
                    if hook.action.type == RequestTypes.PLAYWRIGHT and isinstance(
                        ...
                        await self.logger.filesystem.aio["hyperscale.core"].info(
                            f"{self.metadata_string} - Setting up Playwright Session"
                        )

                        await self.logger.filesystem.aio["hyperscale.core"].debug(
                            f"{self.metadata_string} - Playwright Session - {hook.session.session_id} - Browser Type: {persona_config.browser_type}"
                        )
                        await self.logger.filesystem.aio["hyperscale.core"].debug(
                            f"{self.metadata_string} - Playwright Session - {hook.session.session_id} - Device Type: {persona_config.device_type}"
                        )
                        await self.logger.filesystem.aio["hyperscale.core"].debug(
                            f"{self.metadata_string} - Playwright Session - {hook.session.session_id} - Locale: {persona_config.locale}"
                        )
                        await self.logger.filesystem.aio["hyperscale.core"].debug(
                            f"{self.metadata_string} - Playwright Session - {hook.session.session_id} - geolocation: {persona_config.geolocation}"
                        )
                        await self.logger.filesystem.aio["hyperscale.core"].debug(
                            f"{self.metadata_string} - Playwright Session - {hook.session.session_id} - Permissions: {persona_config.permissions}"
                        )
                        await self.logger.filesystem.aio["hyperscale.core"].debug(
                            f"{self.metadata_string} - Playwright Session - {hook.session.session_id} - Color Scheme: {persona_config.color_scheme}"
                        )
            ...
            return {"optimize_stage_candidates": optimize_stage_candidates}
    ...
    async def execute_single_worker_optimization(
        ...
        if optimize_stage_has_multiple_workers is False:
            for stage_name, persona_config in setup_stage_configs.items():
                ...
                if persona_config.experiment and persona_config.experiment.get(
                    "distribution"
                ):
            ...
            results: List[Dict[str, Union[int, str, float]]] = await asyncio.gather(
                *[
                    self._loop.run_in_executor(
                        self._thread_executor, optimizer.optimize
                    )
            ...
            for optimization_result in results:
                ...
                if stage_config.experiment and optimization_result.get(
                    "optimized_distribution"
                ):
                    stage_config.experiment["distribution"] = optimization_result.get(
                        "optimized_distribution"
                    )

                stage_config.batch_size = optimization_result.get(
                    "optimized_batch_size", stage_config.batch_size
                )
                stage_config.batch_interval = optimization_result.get(
                    "optimized_batch_interval", stage_config.batch_interval
                )
                stage_config.batch_gradient = optimization_result.get(
                    "optimized_batch_gradient", stage_config.batch_gradient
                )

                await self.logger.filesystem.aio["hyperscale.optimize"].info(
    ...
    async def collect_optimized_batch_sizes(
        ...
        if optimize_stage_has_multiple_workers:
            ...
            optimized_batch_sizes = []
            for optimization_result in optimize_stage_results:
                monitors: Dict[str, MonitorResults] = optimization_result.get(
                    "monitoring", {}
                )
                ...
                for monitor_name, collection_stats in cpu_monitor.items():
                    stage_cpu_monitor.worker_metrics[worker_id][monitor_name] = (
                        collection_stats
                    )
                ...
                for monitor_name, collection_stats in memory_monitor.items():
                    stage_memory_monitor.worker_metrics[worker_id][monitor_name] = (
                        collection_stats
                    )
        ...
        stage_cpu_monitor.stage_metrics[main_monitor_name] = (
            stage_cpu_monitor.collected[main_monitor_name]
        )
        stage_memory_monitor.stage_metrics[main_monitor_name] = (
            stage_memory_monitor.collected[main_monitor_name]
        )
        ...
        for optimization_result in optimize_stage_results:
            ...
            for hook in stage.dispatcher.actions_and_tasks.values():
                if hook.source.session:
                    ...
                    hook.source.session.sem = asyncio.Semaphore(
                        optimize_stage_batch_size
                    )
            ...
            stage.context["execute_stage_setup_hooks"] = list(
                stage.dispatcher.actions_and_tasks.values()
            )
            ...
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Stage - {stage_name} - configured to use optimized batch size of - {optimize_stage_batch_size} - VUs"
            )
    ...
    async def complete_optimization(
        ...
        self.optimization_execution_time = round(
            time.monotonic() - self.optimization_execution_time_start
        )

        optimized_batch_sizes = ", ".join(
            [
                f"{stage_name}: {optimized_batch_size}"
                for stage_name, optimized_batch_size in optimize_stage_optimzations.items()
            ]
        )

        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Optimization complete for stages - {optimize_stage_stage_names} - over - {self.optimization_execution_time} - seconds"
        )
        ...
        return {
            ...
            "optimize_stage_monitors": {self.name: {**optimize_stage_monitors}},
        }
