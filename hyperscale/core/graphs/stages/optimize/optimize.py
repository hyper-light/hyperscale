...
class Optimize(Stage):
    ...
    async def collect_optimization_stages(
        ...
        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Optimizing stages {stage_names} using {self.algorithm} algorithm')
    ...
    async def get_stage_plugins(self):
        ...
        for plugin_name, plugin in optimize_plugins.items():
            ...
            await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Loaded Optimizer plugin - {plugin_name}')
    ...
    async def create_optimization_configs(
        ...
        if optimize_stage_has_multiple_workers:
            ...
            await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Batching optimization for - {optimize_stage_stages_count} stages')
            ...
            for stage_name, stage, assigned_workers_count in optimize_stage_batched_stages:
                ...
                await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Provisioned - {assigned_workers_count} - workers for stage - {stage_name}')
    ...
    async def execute_batched_optimization(
        ...
        if optimize_stage_has_multiple_workers:
            ...
            await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Starting optimizaiton for - {optimize_stage_stages_count} - stages')
            ...
            await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Completed optimizaiton for - {optimize_stage_stages_count} - stages')
            ...
            for stage_name, persona_config in setup_stage_configs.items():
                ...
                for hook in action_and_task_hooks:
                    if hook.action.type == RequestTypes.PLAYWRIGHT and isinstance(hook.session, MercuryPlaywrightClient):
                        ...
                        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Setting up Playwright Session')

                        await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Playwright Session - {hook.session.session_id} - Browser Type: {persona_config.browser_type}')
                        await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Playwright Session - {hook.session.session_id} - Device Type: {persona_config.device_type}')
                        await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Playwright Session - {hook.session.session_id} - Locale: {persona_config.locale}')
                        await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Playwright Session - {hook.session.session_id} - geolocation: {persona_config.geolocation}')
                        await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Playwright Session - {hook.session.session_id} - Permissions: {persona_config.permissions}')
                        await self.logger.filesystem.aio['hyperscale.core'].debug(f'{self.metadata_string} - Playwright Session - {hook.session.session_id} - Color Scheme: {persona_config.color_scheme}')
    ...
    async def execute_single_worker_optimization(
        ...
        if optimize_stage_has_multiple_workers is False:
            ...
            for optimization_result in results:
                ...
                await self.logger.filesystem.aio['hyperscale.optimize'].info(f'{optimizer.metadata_string} - Optimization complete')
    ...
    async def set_optimized_configs(
        ...
        for optimization_result in optimize_stage_results:
            ...
            await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Stage - {stage_name} - configured to use optimized batch size of - {optimize_stage_batch_size} - VUs')
    ...
    async def complete_optimization(
        ...
        await self.logger.filesystem.aio['hyperscale.core'].info(f'{self.metadata_string} - Optimization complete for stages - {optimize_stage_stage_names} - over - {self.optimization_execution_time} - seconds')
