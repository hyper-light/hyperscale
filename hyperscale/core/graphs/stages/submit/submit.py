...
from hyperscale.reporting import Reporter
...
from hyperscale.reporting.system.system_metrics_group import SystemMetricsGroup
from hyperscale.reporting.system.system_metrics_set_types import SystemMetricGroupType
...
class Submit(Stage, Generic[T]):
    ...
    def __init__(self) -> None:
        ...
        self.source_internal_events = ["collect_process_results_and_metrics"]
    ...
    async def collect_process_results_and_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Initializing results submission"
        )
        ...
        return {
            ...
            "submit_stage_monitors": {"cpu": cpu_monitor, "memory": memory_monitor},
    ...
    async def collect_reporter_plugins(self):
        ...
        for plugin_name, plugin in reporter_plugins.items():
            ...
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Loaded Reporter plugin - {plugin_name}"
            )

        return {"submit_stage_reporter_plugins": reporter_plugins}
    ...
    async def initialize_reporter(self):
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Submitting results via - {reporter_name}:{reporter.reporter_id} - reporter"
        )
    ...
    async def check_for_streams(
        ...
        return {"submit_stage_has_streams": len(submit_stage_streamed_metrics) > 0}
    ...
    async def submit_streamed_metrics(
        ...
        if submit_stage_has_streams:
            ...
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitting - {submit_stage_streams_count} - Streams"
            )
            await submit_stage_reporter.submit_streams(submit_stage_streamed_metrics)

            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitted - {submit_stage_streams_count} - Streams"
            )
    ...
    async def submit_experiment_results(
        ...
        if submit_stage_has_experiments:
            ...
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitting - {submit_stage_experiments_count} - Experiments"
            )
            await submit_stage_reporter.submit_experiments(
                submit_stage_experiment_metrics
            )

            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitted - {submit_stage_experiments_count} - Experiments"
            )

    @condition("initialize_reporter")
    async def check_for_events(self, analyze_stage_events: List[Any] = []):
        return {"submit_stage_has_events": len(analyze_stage_events) > 0}
    ...
    async def submit_processed_results(
        ...
        if submit_stage_has_events and self.stream is False:
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitting - {submit_stage_session_total} - Events"
            )
            await submit_stage_reporter.submit_events(submit_stage_events)

            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitted - {submit_stage_session_total} - Events"
            )
    ...
    async def submit_stage_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitting Common Metrics"
        )
    ...
    async def submit_main_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitting Metrics"
        )
    ...
    async def submit_error_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitting Error Metrics"
        )
    ...
    async def submit_custom_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitting Custom Metrics"
        )
    ...
    async def close_stage_system_monitors(
        ...
        system_metrics = SystemMetricsSet(submit_stage_monitors, {})
        ...
        for metrics_set in submit_stage_system_metrics:
            ...
            metrics_set.cpu = SystemMetricsGroup(
                metrics_set.cpu_metrics_by_stage, SystemMetricGroupType.CPU
            )

            metrics_set.memory = SystemMetricsGroup(
                metrics_set.memory_metrics_by_stage, SystemMetricGroupType.MEMORY
            )
    ...
    async def submit_system_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Submitting System Metrics"
        )
    ...
    async def complete_submit_session(
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Reporter - {submit_stage_reporter_name}:{submit_stage_reporter.reporter_id} - Completed Metrics submission"
        )
        await self.logger.spinner.set_default_message(
            f"Successfully submitted the results for {submit_stage_session_total} actions via {submit_stage_reporter_name} reporter"
        )
