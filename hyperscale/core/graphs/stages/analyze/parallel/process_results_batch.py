...
def process_results_batch(config: Dict[str, Any]):
    ...
    try:
        ...
        logger.filesystem.sync.create_logfile('hyperscale.reporting.log')
        
        logger.filesystem.sync['hyperscale.core'].info(f'{metadata_string} - Initializing results aggregation')
        ...
        for events_stage_name, events_group in events.items():  
            ...
            logger.filesystem.sync['hyperscale.reporting'].debug(
        ...
        logger.filesystem.sync['hyperscale.core'].info(f'{metadata_string} - Results aggregation complete - Took: {round(elapsed, 2)} seconds')
