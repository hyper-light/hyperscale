...
from hyperscale.reporting.processed_result import ProcessedResultsGroup, results_types
from hyperscale.reporting.processed_result.types import (
...
from hyperscale.reporting.system import SystemMetricsSet
...
def handle_loop_stop(
    signame, loop: asyncio.AbstractEventLoop, executor: ThreadPoolExecutor
):
    try:
...
class Analyze(Stage):
    ...
    def __init__(self) -> None:
        ...
        self.source_internal_events = ["initialize_raw_results"]
    ...
    async def initialize_results_analysis(
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Starting results analysis"
        )
        ...
        for plugin_name, plugin in engine_plugins.items():
            ...
            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Generated custom Event - {plugin.event.type} - for Reporter plugin - {plugin_name}"
            )
        ...
        for signame in ("SIGINT", "SIGTERM", "SIG_IGN"):
            self._loop.add_signal_handler(
                ...
                lambda signame=signame: handle_loop_stop(
                    signame, self._loop, self.executor
                ),
            )

    @condition("add_shutdown_handler")
    async def check_if_has_multiple_workers(self):
        return {"analyze_stage_has_multiple_workers": self.total_pool_cpus > 1}
        ...
        if analyze_stage_has_multiple_workers:
            for results_set_name, results_set in analyze_stage_raw_results.items():
                ...
                results_set_copy.results = await self._loop.run_in_executor(
                    self._executor,
                    functools.partial(deserialize_results, results_set.results),
                )

                deserialized_results[results_set_name] = results_set_copy

        return {
            "analyze_stage_monitors": {"cpu": cpu_monitor, "memory": memory_monitor},
            "analyze_stage_raw_results": analyze_stage_raw_results,
    ...
    async def partition_results_batches(
        ...
        for stage_name, _, _ in batches:
            ...
            elapsed_times.append(stage_results.total_elapsed)

        await self.logger.filesystem.aio["hyperscale.core"].debug(
            f"{self.metadata_string} - Paritioned {len(batches)} batches of results"
        )
    ...
    async def create_stage_batches(
        ...
        for stage_name, _, assigned_workers_count in analyze_stage_batches:
            ...
            stage_streamed_analytics[stage_name] = (
                stage_results.stage_streamed_analytics
            )
            ...
            for worker_idx in range(assigned_workers_count):
                ...
                stage_batches.append(results[batch_marker : batch_marker + batch_size])
    ...
    async def assign_stage_batches(
        ...
        if analyze_stage_has_multiple_workers:
            ...
            for stage_name, _, assigned_workers_count in analyze_stage_batches:
                        ...
                        [
                            {
                                ...
                                "analyze_stage_metric_hooks": list(
                                    analyze_stage_metric_hook_names
                                ),
                ...
                worker_idx += 1
                await self.logger.filesystem.aio["hyperscale.core"].debug(
                    f"{self.metadata_string} - Assigned {assigned_workers_count} to process results from stage - {stage_name}"
                )
    ...
    async def check_if_multiple_stages(
        ...
        return {"multiple_stages_to_process": len(analyze_stage_configs) > 1}
    ...
    async def execute_batched_analysis(
        ...
        if multiple_stages_to_process and analyze_stage_has_multiple_workers:
            ...
            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Processing results or - {analyze_stage_stages_count} - stages"
            )

            median_execution_time = round(
                statistics.median(analyze_stage_elapsed_times)
            )
            await self.logger.spinner.append_message(
                f"Calculating stats for - {analyze_stage_total_group_results} - actions executed over a median stage execution time of {median_execution_time} seconds"
            )

            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Calculating stats for - {analyze_stage_total_group_results} - actions over a median stage execution time of {median_execution_time} seconds"
            )
            stage_batch_results = await self.executor.execute_batches(
                analyze_stage_configs, process_results_batch
            )

            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Completed parital results aggregation for - {analyze_stage_stages_count} - stages"
            )

            return {"analyze_stage_batch_results": stage_batch_results}
            ...
            for results_set in analyze_stage_deserialized_results.values():
                ...
                batch_results.append((results_set.stage, events))

                for events_group in events.values():
                    events_group.calculate_stats()

            return {"analyze_stage_batch_results": batch_results}
        ...
        for metric in metrics:
            for context_key, context_value in metric.context:
                if isinstance(context_value, CustomMetric):
                    custom_metrics_set[context_value.metric_group][context_key] = (
                        context_value
                    )

        return {"analyze_stage_custom_metrics_set": custom_metrics_set}
    ...
    async def generate_metrics_sets(
        ...
        for stage_name, stage_events in analyze_stage_events_set.items():
            ...
            stage_metrics_summary = StageMetricsSummary(
                ...
                stage_streamed_analytics=analyze_stage_streamed_analytics.get(
                    stage_name
                ),
            )
            grouped_stats = {}
            ...
            for event_group_name, events_group in stage_events.items():
                custom_metrics = analyze_stage_custom_metrics_set.get(
                    event_group_name, {}
                )
                ...
                metric_data = {
                    ...
                    "actions_per_second": round(
                        events_group.total / stage_total_time, 2
                    ),
                    "errors": list(
                        [
                            {"message": error_message, "count": error_count}
                ...
                await self.logger.filesystem.aio["hyperscale.core"].info(
                    f"{self.metadata_string} - Convererted stats for stage - {stage_name} to metrics set"
                )

            stage_metrics_summary.calculate_action_and_task_metrics()
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Calculated results for - {stage_metrics_summary.stage_metrics.total} - actions from stage - {stage_name}"
            )

            processed_results.append((stage_name, stage_metrics_summary))

        return {"analyze_stage_processed_results": processed_results}
    ...
    async def generate_experiment_metrics(
        ...
        for stage_name, stage_metrics in analyze_stage_processed_results:
            ...
            if experiment:
                ...
                if experiment_metrics_set.experiment_name is None:
                    experiment_metrics_set.experiment_name = experiment.get(
                        "experiment_name"
                    )

                if experiment_metrics_set.randomized is None:
                    experiment_metrics_set.randomized = experiment.get(
        ...
        return {"experiment_metrics_sets": experiment_metrics_sets}
    ...
    async def generate_system_metrics(
        ...
        session_stage_monitors.update({self.name: {**analyze_stage_monitors}})

        system_metrics_set = SystemMetricsSet(
            session_stage_monitors, analyze_stage_batch_sizes
        )
        system_metrics_set.generate_system_summaries()

        return {"analyze_stage_system_metrics": system_metrics_set}

    @context("generate_experiment_metrics", "generate_system_metrics")
    async def generate_summary(
        ...
        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Completed results analysis for - {analyze_stage_stages_count} - stages in - {self.analysis_execution_time} seconds"
        )
        await self.logger.spinner.set_default_message(
            f"Completed results analysis for {analyze_stage_total_group_results} actions and {analyze_stage_stages_count} stages over {self.analysis_execution_time} seconds"
        )
        ...
        return {
            ...
            "analyze_stage_monitors": {self.name: {**analyze_stage_monitors}},
        }
