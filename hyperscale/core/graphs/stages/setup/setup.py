...
from hyperscale.core.engines.client.client import Client
from hyperscale.core.engines.client.config import Config
from hyperscale.core.engines.client.tracing_config import TracingConfig
from hyperscale.core.engines.types.common.action_registry import actions_registry
from hyperscale.core.experiments.experiment import Experiment
        ...
        for _ in range(self.retries):
            try:
                ...
                await self.logger.filesystem.aio["hyperscale.core"].error(
                    f"{self.metadata_string} - Encountered connection validation error - {str(setup_exception)} - {self.hook_name} Hook: {self.hook.name}:{self.hook.hook_id}"
                )
...
class Setup(Stage, Generic[Unpack[T]]):
    ...
    def __init__(self) -> None:
        ...
        self.client = Client(self.graph_name, self.graph_id, self.name, self.stage_id)
        ...
        self.source_internal_events = ["collect_target_stages"]
    ...
    async def run_internal(self):
        ...
        self.dispatcher.skip_list.extend(
            [
                ...
                for stage_event in self.dispatcher.events_by_name.values()
                if stage_event.source.shortname not in self.internal_events
            ]
        )
    ...
    async def collect_target_stages(
        ...
        bypass_connection_validation = self.core_config.get(
            "bypass_connection_validation", False
        )
        connection_validation_retries = self.core_config.get(
            "connection_validation_retries", 3
        )

        self.context.ignore_serialization_filters = ["session_stage_monitors"]

        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Starting setup"
        )
        ...
        return {
            ...
            "setup_stage_monitors": {"cpu": cpu_monitor, "memory": memory_monitor},
    ...
    async def configure_target_stages(
        ...
        for execute_stage_name, execute_stage in setup_stage_target_stages.items():
            ...
            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Execute stage - {execute_stage_name} - assigned stage order id - {execute_stage_id}"
            )

            execute_stage_id += 1

            persona_plugins: Dict[str, PersonaPlugin] = self.plugins_by_type[
                PluginType.PERSONA
            ]
            for plugin_name, plugin in persona_plugins.items():
                ...
                await self.logger.filesystem.aio["hyperscale.core"].info(
                    f"{self.metadata_string} - Loaded Persona plugin - {plugin.name} - for Execute stae - {execute_stage_name}"
                )

            engine_plugins: Dict[str, EnginePlugin] = self.plugins_by_type[
                PluginType.ENGINE
            ]

            for plugin_name, plugin in engine_plugins.items():
                execute_stage.client.plugin[plugin_name] = plugin(
                    setup_stage_target_config
                )
                plugin.name = plugin_name
                execute_stage.plugins[plugin_name] = plugin
                ...
                await self.logger.filesystem.aio["hyperscale.core"].info(
                    f"{self.metadata_string} - Loaded Engine plugin - {plugin.name} - for Execute stage - {execute_stage_name}"
                )
            ...
            if self.experiment and self.experiment.is_variant(execute_stage_name):
                ...
                if setup_stage_is_primary_thread:
                    ...
                    distribution = self.experiment.calculate_distribution(
                        execute_stage_name, config_copy.batch_size
                    )
                        ...
                        experiment.update(
                            {
                                ...
                                "interval_duration": round(
                                    config_copy.total_time / (variant.intervals - 1), 2
                                ),
                            }
                        )
    ...
    async def collect_action_hooks(
        self, setup_stage_target_stages: Dict[str, Stage] = {}
    ):
        ...
        return {"setup_stage_actions": actions}

    @condition("collect_action_hooks")
    async def check_actions_setup_needed(
        self, setup_stage_actions: List[ActionHook] = []
    ):
        return {"setup_stage_has_actions": len(setup_stage_actions) > 0}
    ...
    async def setup_action(
        ...
        if (
            setup_stage_has_actions
            and isinstance(setup_stage_actions, ActionHook)
            and setup_stage_actions.skip is False
        ):
            ...
            config_copy = setup_stage_configs.get(hook.stage)
            ...
            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Client intercept set to {hook.stage_instance.client.intercept} - Action calls for client id - {hook.stage_instance.client.client_id} - will be suspended on execution"
            )

            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Setting up Action - {hook.name}:{hook.hook_id} - for Execute stage - {execute_stage_name}"
            )
            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Preparing Action hook - {hook.name}:{hook.hook_id} - for suspension - Execute stage - {execute_stage_name}"
            )

            hook.stage_instance.client.actions.set_waiter(hook.stage_instance.name)

            setup_call = SetupCall(
                hook, config_copy, retries=connection_validation_retries
            )
            ...
            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Executing Action hook call - {hook.name}:{hook.hook_id} - Execute stage - {execute_stage_name}"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Exiting suspension for Action - {hook.name}:{hook.hook_id} - Execute stage - {execute_stage_name}"
            )
            ...
            try:
                if setup_call.exception:
                    raise HookSetupError(
                        hook, HookType.ACTION, str(setup_call.exception)
                    )
            ...
            action, session = hook.stage_instance.client.actions.get(
                hook.stage_instance.name, hook.name
            )

            await session.set_pool(setup_stage_target_config.batch_size)

            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Successfully retrieved prepared Action and Session for action - {hook.name}:{action.action_id} - Execute stage - {execute_stage_name}"
            )
            ...
            return {"setup_stage_actions": setup_stage_actions}
    ...
    async def collect_task_hooks(
        ...
        return {"setup_stage_tasks": tasks}

    @condition("collect_task_hooks")
    async def check_tasks_setup_needed(self, setup_stage_tasks: List[ActionHook] = []):
        return {"setup_stage_has_tasks": len(setup_stage_tasks) > 0}
    ...
    async def setup_task(
        ...
        if (
            setup_stage_has_tasks
            and isinstance(setup_stage_tasks, TaskHook)
            and setup_stage_tasks.skip is False
        ):
            ...
            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Loading Task hook - {hook.name}:{hook.hook_id} - to Execute stage - {execute_stage_name}"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Successfully retrieved task and session for Task - {hook.name}:{task.action_id} - Execute stage - {execute_stage_name}"
            )
            ...
            return {"setup_stage_tasks": setup_stage_tasks}
    ...
    async def apply_channels(
        ...
        for execute_stage in setup_stage_target_stages.values():
            ...
            await self.logger.filesystem.aio["hyperscale.core"].debug(
                f"{self.metadata_string} - Client intercept set to {execute_stage.client.intercept} - Action calls for client id - {execute_stage.client.client_id} - will not be suspended on execution"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Generated - {actions_generated_count} - Actions for Execute stage - {execute_stage.name}"
            )

            tasks_generated_count = len(execute_stage.hooks[HookType.TASK])
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Generated - {tasks_generated_count} - Tasks for Execute stage - {execute_stage.name}"
            )
        ...
        return {
            ...
            "setup_stage_monitors": {
                self.name: {"cpu": stage_cpu_monitor, "memory": stage_memory_monitor}
            },
        }
