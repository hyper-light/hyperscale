...
class BatchExecutor:
    def __init__(
        ...
        self.pool = ProcessPoolExecutor(
            max_workers=max_workers, mp_context=self.context
        )
        self.shutdown_task = None
        self.batch_by_stages = False

    async def execute_batches(
        self, batched_stages: List[Tuple[int, List[Any]]], execution_task: FunctionType
    ) -> List[Tuple[str, Any]]:
    ...
    def partion_prioritized_stage_batches(
        self,
        ...
        for transition in sorted_transitions:
            if (
                transition.edge.skip_stage
                or transition.edge.source.allow_parallel is False
            ):
        ...
        stages = {
            transition.edge.source.name: transition.edge.source
            for transition in sorted_transitions
        }
        ...
        auto_stages_count = len(
            [
                ...
                if stage.priority_level == StagePriority.AUTO
            ]
        )
        ...
        if parallel_stages_count == 1:
            parallel_transitions = [
                transition
                for transition in sorted_transitions
                if transition.edge.skip_stage is False
            ]
            ...
            for transition in sorted_transitions:
                if (
                    transition.edge.skip_stage is False
                    and transition.edge.source.allow_parallel
                ):
                    worker_allocation_range: Tuple[int, int] = (
                        StagePriority.get_worker_allocation_range(
                            transition.edge.source.priority_level, self.max_workers
                        )
                    )
            ...
            for transition in sorted_transitions:
                if transition not in seen_transitions:
                    ...
                    batch_workers_allocated = max_workers_counts.get(
                        transition.edge.source.name
                    )
                    transition_group = [
                        (
                    ...
                    for other_transition in sorted_transitions:
                        if (
                            ...
                            and transition not in seen_transitions
                        ):
                            transition_workers_allocated = max_workers_counts.get(
                                other_transition.edge.source.name
                            )
                            min_workers = min_workers_counts.get(
                                other_transition.edge.source.name
                            )
                            ...
                            while (
                                ...
                                and transition_workers_allocated >= min_workers
                            ):
                            ...
                            if (
                                current_allocation <= self.max_workers
                                and transition_workers_allocated > 0
                            ):
                                batch_workers_allocated += transition_workers_allocated
