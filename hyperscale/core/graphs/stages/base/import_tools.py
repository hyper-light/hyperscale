...
def import_plugins(path: str) -> Dict[PluginType, Dict[str, Plugin]]:
    ...
    for name, plugin_candidate in inspect.getmembers(module):
        if (
            inspect.isclass(plugin_candidate)
            and issubclass(plugin_candidate, Plugin)
            and plugin_candidate not in direct_decendants
        ):
            plugins_by_type[plugin_candidate.type][plugin_candidate.name] = (
                plugin_candidate
            )
            ...
            if generated_hooks.get(hook) is None:
                ...
                for (
                    ...
                    if inspect.ismethod(copied_attribute_value) is False:
                        setattr(
                            copied_stage, copied_attribute_name, copied_attribute_value
                        )
