...
from hyperscale.core.personas.persona_registry import (
    DefaultPersona,
    get_persona,
    registered_personas,
)
...
class Execute(Stage, Generic[Unpack[T]]):
    ...
    def __init__(self) -> None:
        ...
        self.source_internal_events = ["get_stage_config"]
    ...
    async def get_stage_config(
        ...
        persona_type_name = "-".join(
            [
                segment.capitalize()
                for segment in execute_stage_setup_config.persona_type.split("-")
            ]
        )

        await self.logger.filesystem.aio["hyperscale.core"].info(
            f"{self.metadata_string} - Executing - {execute_stage_setup_config.batch_size} - VUs over {self.workers} threads for {execute_stage_setup_config.total_time_string} using - {persona_type_name} - persona"
        )
        await self.logger.spinner.append_message(
            f"Stage {self.name} executing - {execute_stage_setup_config.batch_size} - VUs over {self.workers} threads for {execute_stage_setup_config.total_time_string} using - {persona_type_name} - persona"
        )
        ...
        return {
            ...
            "execute_stage_monitors": {"cpu": cpu_monitor, "memory": memory_monitor},
        }
    ...
    async def collect_loaded_actions(self):
        ...
        self.context.ignore_serialization_filters.extend(list(ignore_context_keys))

        return {"execute_stage_loaded_actions": loaded_actions}
    ...
    async def get_stage_plugins(self):
        ...
        for plugin_name, plugin in engine_plugins.items():
            ...
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Loaded Engine plugin - {plugin.name}"
            )

        persona_plugins = self.plugins_by_type.get(PluginType.PERSONA)
        for plugin_name, plugin in persona_plugins.items():
            registered_personas[plugin_name] = lambda plugin_config: plugin(
                plugin_config
            )
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Loaded Persona plugin - {plugin.name}"
            )
            ...
            return {
                "execute_stage_experiment": {
                    ...
                    "experiment_variant": {
                        ...
                        "variant_distribution_type": experiment.get(
                            "distribution_type"
                        ),
                        "variant_distribution_interval_duration": experiment.get(
                            "interval_duration"
                        ),
    ...
    @condition("collect_loaded_actions", "get_stage_plugins", "get_stage_experiment")
    async def check_has_multiple_workers(self):
        return {"execute_stage_has_multiple_workers": self.total_pool_cpus > 1}
    ...
    async def run_multiple_worker_jobs(
        ...
        for action_hook in execute_stage_loaded_actions:
            loaded_actions.append(self.serializer.serialize_action(action_hook))

        if execute_stage_has_multiple_workers:
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Starting execution for - {self.workers} workers"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Completed execution for - {self.workers} workers"
            )

            return {"execute_stage_results": results_sets}
    ...
    async def aggregate_multiple_worker_results(
        ...
        if execute_stage_has_multiple_workers:
            ...
            await self.logger.filesystem.aio["hyperscale.core"].info(
                f"{self.metadata_string} - Completed - {total_results} actions at  {round(total_results/total_elapsed)} actions/second over {round(total_elapsed)} seconds"
            )
    ...
    async def setup_single_worker_job(
        ...
        if execute_stage_has_multiple_workers is False:
            ...
            return {"execute_stage_persona": persona}
    ...
    async def run_single_worker_job(
        ...
        if execute_stage_has_multiple_workers is False:
            ...
            stage_cpu_monitor.trim_monitor_samples(main_monitor_name, elapsed_idx)

            stage_memory_monitor.trim_monitor_samples(main_monitor_name, elapsed_idx)
