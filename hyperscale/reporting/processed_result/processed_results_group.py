...
from hyperscale.reporting.stats import Mean, Median, StandardDeviation, Variance
...
class ProcessedResultsGroup:
    ...
    def add(
        ...
        processed_result: BaseProcessedResult = results_types.get(
            result.type, TaskProcessedResult
        )(stage_name, result)
    ...
    def calculate_quantiles(self):
        ...
        for group_name, group_timings in self.timings.items():
            ...
            quantiles = {
                f"quantile_{int(quantile_range * 100)}th": quantile
                for quantile, quantile_range in zip(
                    numpy.quantile(numpy.array(group_timings), quantile_ranges),
                    quantile_ranges,
                )
            }
