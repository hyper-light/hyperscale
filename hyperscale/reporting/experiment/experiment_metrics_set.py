...
from hyperscale.core.experiments.distribution_types import DistributionTypes
...
class ExperimentMetricsSet:
    ...
    def variants_fields(cls):
        return list(
            set(
                [*cls.variants_table_header_keys, *cls.variants_stats_table_header_keys]
            )
        )
    ...
    def generate_variant_summaries(self) -> Dict[str, VariantSummary]:
        ...
        for participant in self.participants:
            ...
            for variant_metric_set in metrics_set.values():
                group_variant_summaries["variant_completed"] += (
                    variant_metric_set.common_stats.get("total")
                )
                group_variant_summaries["variant_succeeded"] += (
                    variant_metric_set.common_stats.get("succeeded")
                )
                group_variant_summaries["variant_failed"] += (
                    variant_metric_set.common_stats.get("failed")
                )
            ...
            variant_distribution_type: DistributionTypes = variant.get(
            ...
            variant_summary = VariantSummary(
                **{
                    ...
                    "variant_distribution_interval": variant.get(
                ...
                for mutation in mutations:
                    ...
                    mutations_summary = MutationSummary(
                        **{
                            ...
                            "mutation_targets": ":".join(
                                mutation.get("mutation_targets")
                            ),
                    ...
                    self.mutations_table_headers.update(
                        {
                            header_name: header_name.replace("mutation_", "").replace(
                                "_", " "
                            )
                    ...
                    variant_summary.variant_mutation_summaries[mutation_name] = (
                        mutations_summary
                    )
    ...
    def generate_experiment_summary(self) -> None:
        ...
        for participant in self.participants:
            ...
            selected_variant_summary.variant_ratio_completed = round(
                variant_total_completed / mean_alt_variants_total_completed, 2
            )
            ...
            selected_variant_summary.variant_ratio_succeeded = round(
                variant_total_succeeded / mean_alt_variants_total_succeeded, 2
            )
            ...
            selected_variant_summary.variant_ratio_failed = round(
                variant_total_failed / mean_alt_variants_total_failed, 2
            )

            variant_total_actions_per_second = (
            ...
            selected_variant_summary.variant_ratio_aps = round(
                variant_total_actions_per_second
                / median_alt_variants_actions_per_second,
                2,
            )

            self.variants_table_headers.update(
                {
                    header_name: header_name.replace("variant_", "").replace("_", " ")
                    for header_name in selected_variant_summary.dict().keys()
                    if "mutation" not in header_name
                }
            )

            experiment_summary.experiment_variant_summaries[participant] = (
                selected_variant_summary
            )

            experiment_summary.experiment_completed += (
                selected_variant_summary.variant_completed
            )
            experiment_summary.experiment_succeeded += (
                selected_variant_summary.variant_succeeded
            )
            experiment_summary.experiment_failed += (
                selected_variant_summary.variant_failed
            )
        ...
        experiment_summary.experiment_median_aps = statistics.median(
            experiment_actions_per_second
        )

        self.experiments_table_headers = {
            header_name: header_name.replace("experiment_", "").replace("_", " ")
            for header_name in experiment_summary.dict().keys()
            if "variant" not in header_name
        }
    ...
    def split_experiments_metrics(self) -> ExperimentMetricsCollection:
        ...
        for variant in self.experiments_summary.experiment_variant_summaries.values():
            variant_records.append(
                variant.dict(include={header for header in self.variants_fields()})
            )
        ...
        return ExperimentMetricsCollection(
            ...
            variant_summaries=list(
                self.experiments_summary.experiment_variant_summaries.values()
            ),
