...
class Reporter:
    ...
    async def connect(self):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Connecting')
        await self.selected_reporter.connect()

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Connected')

    async def submit_experiments(self, experiment_metrics_sets: List[ExperimentMetricsSet]):
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting {len(experiment_metrics_sets)} experiments')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted {len(experiments)} experiments')

    async def submit_streams(self, stream_metrics: Dict[str, List[StreamAnalytics]]):
        
        streams_count = len(stream_metrics)

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting {streams_count} streams')
        await self.selected_reporter.submit_streams(stream_metrics)

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted {streams_count} streams')

    async def submit_common(self, metrics: List[Any]):
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting {len(metrics)} shared metrics')
        await self.selected_reporter.submit_common(metrics)

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted {len(metrics)} shared metrics')

    async def submit_events(self, events: List[Any]):
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting {len(events)} events')
        await self.selected_reporter.submit_events(events)

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted {len(events)} events')

    async def submit_metrics(self, metrics: List[Any]):
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting {len(metrics)} metrics')
        await self.selected_reporter.submit_metrics(metrics)

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted {len(metrics)} metrics')

    async def submit_custom(self, metrics: List[Any]):
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting {len(metrics)} custom metrics')
        await self.selected_reporter.submit_custom(metrics)

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted {len(metrics)} custom metrics')

    async def submit_errors(self, metrics: List[Any]):
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting {len(metrics)} errors')
        await self.selected_reporter.submit_errors(metrics)

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted {len(metrics)} errors')

    async def submit_system_metrics(self, metrics: List[Any]):
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting {len(metrics)} system metrics sets')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted {len(metrics)} system metrics sets')

    async def close(self):
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Closing')
        await self.selected_reporter.close()

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Closed')