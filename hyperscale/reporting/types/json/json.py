...
class JSON:
    ...
    async def submit_session_system_metrics(
        ...
        if self.system_metrics_file is None:
            self.system_metrics_file = await self._loop.run_in_executor(
                self._executor,
                functools.partial(open, self.system_metrics_filepath, self.write_mode),
            )
            ...
            for signame in ("SIGINT", "SIGTERM", "SIG_IGN"):
                self._loop.add_signal_handler(
                    ...
                    lambda signame=signame: handle_loop_stop(
                        signame, self._executor, self._loop, self.system_metrics_file
                    ),
                )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saving Session System Metrics to file - {self.system_metrics_filepath}"
        )
        ...
        for metrics_set in system_metrics_sets:
            ...
            for stage_name, stage_cpu_metrics in cpu_metrics.metrics.items():
                ...
                if stage_mb_per_vu_metrics:
                    metrics_sets[stage_name]["mb_per_vu"] = (
            ...
            for (
                monitor_name,
                monitor_metrics,
            ) in metrics_set.session_memory_metrics.items():
                metrics_sets["session"]["memory"][monitor_name] = monitor_metrics.record

        await self._loop.run_in_executor(
            self._executor,
            functools.partial(
                json.dump, metrics_sets, self.system_metrics_file, indent=4
            ),
        )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saved Session System Metrics to file - {self.system_metrics_filepath}"
        )
    ...
    async def submit_streams(self, stream_metrics: Dict[str, StageStreamsSet]):
        if self.streams_file is None:
            self.streams_file = await self._loop.run_in_executor(
                self._executor,
                functools.partial(open, self.streams_filepath, self.write_mode),
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saving Streams to file - {self.streams_filepath}"
        )

        streams_data = {
            stream_name: stream_set.grouped
            for stream_name, stream_set in stream_metrics.items()
        }

        await self._loop.run_in_executor(
            self._executor,
            functools.partial(json.dump, streams_data, self.streams_file, indent=4),
        )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saved Streams to file - {self.streams_filepath}"
        )
    ...
    async def submit_experiments(
        ...
        if self.experiments_file is None:
            self.experiments_file = await self._loop.run_in_executor(
                self._executor, functools.partial(open, self.experiments_filepath, "w")
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saved Experiments to file - {self.experiments_filepath}"
        )
    ...
    async def submit_events(self, events: List[BaseProcessedResult]):
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saving Events to file - {self.events_filepath}"
        )

        event_records = {event.event_id: event.record for event in events}

        if self.write_mode == "a+":
            try:
                existing_data = await self._loop.run_in_executor(
                    self._executor, functools.partial(json.load, self.events_file)
                )
        ...
        await self._loop.run_in_executor(
            self._executor,
            functools.partial(json.dump, event_records, self.events_file, indent=4),
        )
    ...
    async def submit_common(self, metrics_sets: List[MetricsSet]):
        await self.logger.filesystem.aio["hyperscale.reporting"].debug(
            f"{self.metadata_string} - Skipping Shared Metrics"
        )
    ...
    async def submit_metrics(self, metrics: List[MetricsSet]):
        ...
        if self.metrics_file is None:
            self.metrics_file = await self._loop.run_in_executor(
                self._executor,
                functools.partial(open, self.metrics_filepath, self.write_mode),
            )

        await self._loop.run_in_executor(
            self._executor,
            functools.partial(json.dump, records, self.metrics_file, indent=4),
        )
