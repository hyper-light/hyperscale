...
class Datadog:
    ...
    async def connect(self):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Connecting to Datadogg API')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Connected to Datadogg API')

    async def submit_session_system_metrics(self, system_metrics_sets: List[SystemMetricsSet]):

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Session System Metrics to Datadog API')
        ...
        for metrics_set in metrics_sets:
            ...
            for field, value in metrics_set.stats.items():
                ...
                await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Creating Session System Metric - {metrics_set.name}:{metrics_set.group}:{field}')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Session System Metrics to Datadog API')

    async def submit_stage_system_metrics(self, system_metrics_sets: List[SystemMetricsSet]):

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Stage System Metrics to Datadog API')
        ...
        for metrics_set in metrics_sets:
            ...
            for field, value in metrics_set.stats.items():
                ...
                await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Creating Stage System Metric - {metrics_set.name}:{metrics_set.group}:{field}')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Stage System Metrics to Datadog API')

    async def submit_streams(self, stream_metrics: Dict[str, StageStreamsSet]):

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Streams to Datadog API')
        ...
        for stage_name, stream in stream_metrics.items():
            ...
            for group_name, group in stream.grouped.items():
                ...
                for field, value in group.items():
                    ...
                    await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Creating Stream Metric - {stage_name}:{group_name}:{field}')
    ...
    async def submit_experiments(self, experiment_metrics: ExperimentMetricsCollectionSet):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Experiments to Datadog API')
        ...
        for experiment in experiment_metrics.experiment_summaries:
            ...
            for field, value in experiment.stats:
                ...
                await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Creating Experiments - {experiment.experiment_name}:{field}')
    ...
    async def submit_variants(self, experiment_metrics: ExperimentMetricsCollectionSet):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Variants to Datadog API')
        ...
        for variant in experiment_metrics.variant_summaries:
            ...
            await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Submitting Variants - {variant.variant_name}:{variant_id}')
            ...
            for field, value in variant.stats:
                ...
                await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Creating Variant - {variant.variant_name}:{field}')
    ...
    async def submit_mutations(self, experiment_metrics: ExperimentMetricsCollectionSet):
        ...
        for mutation in experiment_metrics.mutation_summaries:
            ...
            for field, value in mutation.stats:
                ...
                await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Creating Mutation - {mutation.mutation_name}:{field}')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Mutations to Datadog API')

    async def submit_events(self, events: List[BaseProcessedResult]):

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Events to Datadog API')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Events to Datadog API')

    async def submit_common(self, metrics_sets: List[MetricsSet]):

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Shared Metrics to Datadog API')
        ...
        for metrics_set in metrics_sets:
            ...
            for field, value in metrics_set.common_stats.items():
                ...
                await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Creating Shared Metric - {metrics_set.name}:common:{field}')
    ...
    async def submit_metrics(self, metrics: List[MetricsSet]):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Metrics to Datadog API')
        ...
        for metrics_set in metrics:
            ...
            for group_name, group in metrics_set.groups.items():
                ...
                for field, value in group.stats.items():
                    ...
                    await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Creating Metric - {metrics_set.name}:{group_name}:{field}')
                ...
                for quantile_name, quantile_value in group.quantiles.items():
                    ...
                    await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Creating Metric Quantile - {metrics_set.name}:{group_name}:{quantile_name}th Quantile')
    ...
    async def submit_custom(self, metrics_sets: List[MetricsSet]):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Custom Metrics to Datadog API')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Custom Metrics to Datadog API')

    async def submit_errors(self, metrics_sets: List[MetricsSet]):

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Error Metrics to Datadog API')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Error Metrics to Datadog API')
