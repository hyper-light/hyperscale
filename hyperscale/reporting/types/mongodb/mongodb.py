...
class MongoDB:
    ...
    async def submit_session_system_metrics(self, system_metrics_sets: List[SystemMetricsSet]):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Session System Metrics to Bucket - {self.session_system_metrics_collection}')
    ...
    async def submit_stage_system_metrics(self, system_metrics_sets: List[SystemMetricsSet]):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Stage System Metrics to Bucket - {self.session_system_metrics_collection}')
    ...
    async def submit_streams(self, stream_metrics: Dict[str, StageStreamsSet]):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Streams to Bucket - {self.streams_collection}')
    ...
    async def submit_experiments(self, experiment_metrics: ExperimentMetricsCollectionSet): 
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Experiments to Collection - {self.experiments_collection}')
        await self.database[self.events_collection].insert_many(experiment_metrics.experiments)

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Experiments to Collection - {self.experiments_collection}')

    async def submit_variants(self, experiment_metrics: ExperimentMetricsCollectionSet): 
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Variants to Collection - {self.variants_collection}')
        await self.database[self.events_collection].insert_many(experiment_metrics.variants)

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Variants to Collection - {self.variants_collection}')
    ...
    async def submit_mutations(self, experiment_metrics: ExperimentMetricsCollectionSet): 
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Mutations to Collection - {self.mutations_collection}')

    async def submit_events(self, events: List[BaseProcessedResult]): 
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Events to Collection - {self.events_collection}')
        await self.database[self.events_collection].insert_many(
            [event.record for event in events]
        )

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Events to Collection - {self.events_collection}')

    async def submit_common(self, metrics_sets: List[MetricsSet]):
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Shared Metrics to Bucket - {self.shared_metrics_collection}')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Shared Metrics to Bucket - {self.shared_metrics_collection}')

    async def submit_metrics(self, metrics: List[MetricsSet]):

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Metrics to Bucket - {self.metrics_collection}')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Metrics to Bucket - {self.metrics_collection}')
    ...
    async def submit_custom(self, metrics_sets: List[MetricsSet]):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Custom Metrics to Bucket - {self.custom_metrics_collection}')

        await self.database[self.custom_metrics_collection].insert_many(records)

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Custom Metrics to Bucket - {self.custom_metrics_collection}')

    async def submit_errors(self, metrics: List[MetricsSet]):

        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitting Error Metrics to Bucket - {self.errors_collection}')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Error Metrics to Bucket - {self.errors_collection}')
