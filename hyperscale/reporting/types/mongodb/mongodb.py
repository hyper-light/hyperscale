...
class MongoDB:
    ...
    async def submit_workflow_results(self, workflow_results: WorkflowMetricSet):
        await self.database[self._workflow_results_collection_name].insert_many(workflow_results)

    async def submit_step_results(self, step_results: StepMetricSet):
        await self.database[self._step_results_collection_name].insert_many(step_results)
