...
class Snowflake:
    ...
    async def submit_session_system_metrics(self, system_metrics_sets: List[SystemMetricsSet]):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Submitted Session System Metrics to Table - {self.stage_system_metrics_table_name}')
    ...
    async def close(self):
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Closing connection to Snowflake instance at - Warehouse: {self.warehouse} - Database: {self.database} - Schema: {self.schema}')
        ...
        await self.logger.filesystem.aio['hyperscale.reporting'].debug(f'{self.metadata_string} - Closed session - {self.session_uuid}')
        await self.logger.filesystem.aio['hyperscale.reporting'].info(f'{self.metadata_string} - Closed connection to Snowflake instance at - Warehouse: {self.warehouse} - Database: {self.database} - Schema: {self.schema}')