...
class Redis:
    ...
    async def submit_session_system_metrics(
        ...
        for metrics_set in metrics_sets:
            ...
            if self.channel_type == "channel":
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Submitting Session System Metrics to Channel - {self.session_system_metrics_channel} - Group: {metrics_set.group}"
                )
                await self.connection.publish(
                    self.session_system_metrics_channel, json.dumps(metrics_set.record)
                )

                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Submitted Session System Metrics to Channel - {self.session_system_metrics_channel} - Group: {metrics_set.group}"
                )

            else:
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Submitting Session System Metrics to Redis Set - {self.session_system_metrics_channel} - Group: {metrics_set.group}"
                )
                ...
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Submitted Session System Metrics to Redis Set - {self.session_system_metrics_channel} - Group: {metrics_set.group}"
                )
    ...
    async def submit_stage_system_metrics(
        ...
        for metrics_set in metrics_sets:
            ...
            if self.channel_type == "channel":
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Submitting Stage System Metrics to Channel - {self.stage_system_metrics_channel} - Group: {metrics_set.group}"
                )
                await self.connection.publish(
                    self.stage_system_metrics_channel, json.dumps(metrics_set.record)
                )

                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Submitted Stage System Metrics to Channel - {self.stage_system_metrics_channel} - Group: {metrics_set.group}"
                )

            else:
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Submitting Stage System Metrics to Redis Set - {self.stage_system_metrics_channel} - Group: {metrics_set.group}"
                )
                await self.connection.sadd(
                    self.stage_system_metrics_channel, json.dumps(metrics_set.record)
                )
                ...
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Submitted Stage System Metrics to Redis Set - {self.stage_system_metrics_channel} - Group: {metrics_set.group}"
                )
    ...
    async def submit_streams(self, stream_metrics: Dict[str, StageStreamsSet]):
        for stage_name, stream in stream_metrics.items():
            ...
            for group_name, group in stream.grouped.items():
                if self.channel_type == "channel":
                    await self.logger.filesystem.aio["hyperscale.reporting"].info(
                        f"{self.metadata_string} - Submitting Streams to Channel - {self.streams_channel} - Group: {group_name}"
                    )
                    await self.connection.publish(
                    ...
                    await self.logger.filesystem.aio["hyperscale.reporting"].info(
                        f"{self.metadata_string} - Submitted Streams to Channel - {self.streams_channel} - Group: {group_name}"
                    )

                else:
                    await self.logger.filesystem.aio["hyperscale.reporting"].info(
                        f"{self.metadata_string} - Submitting Streams to Redis Set - {self.streams_channel} - Group: {group_name}"
                    )
                    await self.connection.sadd(
                        self.streams_channel,
                        json.dumps(
                    ...
                    await self.logger.filesystem.aio["hyperscale.reporting"].info(
                        f"{self.metadata_string} - Submitted Streams to Redis Set - {self.streams_channel} - Group: {group_name}"
                    )
    ...
    async def submit_experiments(
        ...
        if self.channel_type == "channel":
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitting Experiments to Channel - {self.experiments_channel}"
            )

            for experiment in experiment_metrics.experiment_summaries:
                await self.connection.publish(
                    self.experiments_channel, json.dumps(experiment.record)
                )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitted Experiments to Channel - {self.experiments_channel}"
            )

        else:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitting Experiments to Redis Set - {self.experiments_channel}"
            )
            for experiment in experiment_metrics.experiment_summaries:
                await self.connection.sadd(
                    self.experiments_channel, json.dumps(experiment.record)
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitted Experiments to Redis Set - {self.experiments_channel}"
            )

    async def submit_variants(self, experiment_metrics: ExperimentMetricsCollectionSet):
        if self.channel_type == "channel":
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitting Variants to Channel - {self.variants_channel}"
            )

            for variant in experiment_metrics.variant_summaries:
                await self.connection.publish(
                    self.variants_channel, json.dumps(variant.record)
                )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitted Variants to Channel - {self.variants_channel}"
            )

        else:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitting Variants to Redis Set - {self.variants_channel}"
            )
            for variant in experiment_metrics.variant_summaries:
                await self.connection.sadd(
                    self.variants_channel, json.dumps(variant.record)
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitted Variants to Redis Set - {self.variants_channel}"
            )
    ...
    async def submit_mutations(
        ...
        if self.channel_type == "channel":
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitting Mutations to Channel - {self.mutations_channel}"
            )

            for mutation in experiment_metrics.mutation_summaries:
                await self.connection.publish(
                    self.mutations_channel, json.dumps(mutation.record)
                )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitted Mutations to Channel - {self.mutations_channel}"
            )

        else:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitting Mutations to Redis Set - {self.mutations_channel}"
            )
            for mutation in experiment_metrics.mutation_summaries:
                await self.connection.sadd(
                    self.mutations_channel, json.dumps(mutation.record)
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitted Mutations to Redis Set - {self.mutations_channel}"
            )

    async def submit_events(self, events: List[BaseProcessedResult]):
        if self.channel_type == "channel":
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitting Events to Channel - {self.events_channel}"
            )

            for event in events:
                await self.connection.publish(
                    self.events_channel, json.dumps(event.record)
                )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitted Events to Channel - {self.events_channel}"
            )

        else:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitting Events to Redis Set - {self.events_channel}"
            )
            for event in events:
                await self.connection.sadd(
                    self.events_channel, json.dumps(events.record)
                )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Submitted Events to Redis Set - {self.events_channel}"
            )
