...
class Cassandra:
    ...
    async def submit_session_system_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Session System Metrics to - Keyspace: {self.keyspace} - Table: {self.session_system_metrics_table_name}"
        )

        if self._session_system_metrics_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Session System Metrics table - {self.session_system_metrics_table_name} - under keyspace - {self.keyspace}"
            )
            ...
            self._session_system_metrics_table = type(
                self.session_system_metrics_table_name.capitalize(), (Model,), fields
            )
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Session System Metrics table - {self.session_system_metrics_table_name} - under keyspace - {self.keyspace}"
            )
        ...
        for metrics_set in rows:
            ...
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    self._session_system_metrics_table.create, **metrics_set.record
                ),
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Shared Metrics to - Keyspace: {self.keyspace} - Table: {self.session_system_metrics_table_name}"
        )
    ...
    async def submit_stage_system_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Stage System Metrics to - Keyspace: {self.keyspace} - Table: {self.stage_system_metrics_table_name}"
        )

        if self._stage_system_metrics_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Stage System Metrics table - {self.stage_system_metrics_table_name} - under keyspace - {self.keyspace}"
            )
            ...
            self._stage_system_metrics_table = type(
                self.stage_system_metrics_table_name.capitalize(), (Model,), fields
            )
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Stage System Metrics table - {self.stage_system_metrics_table_name} - under keyspace - {self.keyspace}"
            )
        ...
        for metrics_set in rows:
            ...
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    self._stage_system_metrics_table.create, **metrics_set.record
                ),
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Shared Metrics to - Keyspace: {self.keyspace} - Table: {self.stage_system_metrics_table_name}"
        )

    async def submit_streams(self, stream_metrics: Dict[str, StageStreamsSet]):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Streams to - Keyspace: {self.keyspace} - Table: {self.streams_table_name}"
        )

        if self._streams_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Streams table - {self.streams_table_name} - under keyspace - {self.keyspace}"
            )
            ...
            self._streams_table = type(
                self.streams_table_name.capitalize(), (Model,), fields
            )

            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    sync_table, self._streams_table, keyspaces=[self.keyspace]
                ),
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Streams table - {self.streams_table_name} - under keyspace - {self.keyspace}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Shared Metrics to - Keyspace: {self.keyspace} - Table: {self.streams_table_name}"
        )
    ...
    async def submit_experiments(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Experiments to - Keyspace: {self.keyspace} - Table: {self.experiments_table_name}"
        )

        if self._experiments_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Experiments table - {self.experiments_table_name} - under keyspace - {self.keyspace}"
            )
            ...
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    sync_table, self._experiments_table, keyspaces=[self.keyspace]
                ),
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Experiments table - {self.experiments_table_name} - under keyspace - {self.keyspace}"
            )

        for experiment in experiment_metrics.experiments:
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(self._metrics_table.create, **experiment),
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Experiments to - Keyspace: {self.keyspace} - Table: {self.experiments_table_name}"
        )

    async def submit_variants(self, experiment_metrics: ExperimentMetricsCollectionSet):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Variants to - Keyspace: {self.keyspace} - Table: {self.variants_table_name}"
        )

        if self._variants_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Variants table - {self.variants_table_name} - under keyspace - {self.keyspace}"
            )
            ...
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    sync_table, self._variants_table, keyspaces=[self.keyspace]
                ),
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Variants table - {self.variants_table_name} - under keyspace - {self.keyspace}"
            )

        for variant in experiment_metrics.variants:
            await self._loop.run_in_executor(
                self._executor, functools.partial(self._metrics_table.create, **variant)
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Variants to - Keyspace: {self.keyspace} - Table: {self.variants_table_name}"
        )
    ...
    async def submit_mutations(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Mutations to - Keyspace: {self.keyspace} - Table: {self.mutations_table_name}"
        )

        if self._mutations_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Mutations table - {self.mutations_table_name} - under keyspace - {self.keyspace}"
            )
            ...
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    sync_table, self._mutations_table, keyspaces=[self.keyspace]
                ),
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Mutations table - {self.mutations_table_name} - under keyspace - {self.keyspace}"
            )

        for mutation in experiment_metrics.mutations:
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(self._metrics_table.create, **mutation),
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Mutations to - Keyspace: {self.keyspace} - Table: {self.mutations_table_name}"
        )

    async def submit_events(self, events: List[BaseProcessedResult]):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Events to - Keyspace: {self.keyspace} - Table: {self.events_table_name}"
        )

        if self._events_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Events table - {self.events_table_name} - under keyspace - {self.keyspace}"
            )
            ...
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    sync_table, self._events_table, keyspaces=[self.keyspace]
                ),
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Events table - {self.events_table_name} - under keyspace - {self.keyspace}"
            )

        for event in events:
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(self._metrics_table.create, **event.record),
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Events to - Keyspace: {self.keyspace} - Table: {self.events_table_name}"
        )

    async def submit_common(self, metrics_sets: List[MetricsSet]):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Shared Metrics to - Keyspace: {self.keyspace} - Table: {self.shared_metrics_table_name}"
        )

        if self._shared_metrics_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Shared Metrics table - {self.shared_metrics_table_name} - under keyspace - {self.keyspace}"
            )
            ...
            self._shared_metrics_table = type(
                self.shared_metrics_table_name.capitalize(), (Model,), fields
            )

            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    sync_table, self._shared_metrics_table, keyspaces=[self.keyspace]
                ),
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Shared Metrics table - {self.shared_metrics_table_name} - under keyspace - {self.keyspace}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Shared Metrics to - Keyspace: {self.keyspace} - Table: {self.shared_metrics_table_name}"
        )
