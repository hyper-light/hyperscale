...
class BigTable:
    def __init__(self, config: BigTableConfig) -> None:
        ...
        self._workflow_results_column_family_id = f"{self._workflow_results_table_id}_columns"
    ...
    async def submit_workflow_results(self, workflow_results: WorkflowMetricSet):
        ...
        self._workflow_results_table = self.instance.table(self._workflow_results_table_id)
        ...
        await self._loop.run_in_executor(
            None, self._workflow_results_table.mutate_rows,
            rows,
        )
