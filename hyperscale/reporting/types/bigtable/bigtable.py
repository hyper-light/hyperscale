...
class BigTable:
    ...
    async def submit_session_system_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Session System Metrics to Table {self.session_system_metrics_table_id}"
        )
        self._session_system_metrics_table = self.instance.table(
            self.session_system_metrics_table_id
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Session System Metrics Table - {self.session_system_metrics_table_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._session_system_metrics_table.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Session System Metrics Table - {self.session_system_metrics_table_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Session System Metrics Column Family - {self.session_system_metrics_table_id} - if not exists"
            )

        self._session_system_metrics_columns = (
            self._session_system_metrics_table.column_family(
                self.session_system_metrics_column_family_id
            )
        )
        ...
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Session System Metrics Column for Column Family - {self.session_system_metrics_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._session_system_metrics_columns.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Session System Metrics Column for Column Family - {self.session_system_metrics_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Session System Metrics Column for Column Family - {self.session_system_metrics_column_family_id} - if not exists"
            )
        ...
        await self._loop.run_in_executor(
            self._executor, self._variants_table.mutate_rows, rows
        )
    ...
    async def submit_stage_system_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Stage System Metrics to Table {self.stage_system_metrics_table_id}"
        )
        self._stage_system_metrics_table = self.instance.table(
            self.stage_system_metrics_table_id
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Stage System Metrics Table - {self.stage_system_metrics_table_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._stage_system_metrics_table.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Stage System Metrics Table - {self.stage_system_metrics_table_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Stage System Metrics Column Family - {self.stage_system_metrics_table_id} - if not exists"
            )

        self._stage_system_metrics_columns = (
            self._stage_system_metrics_table.column_family(
                self.stage_system_metrics_column_family_id
            )
        )
        ...
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Stage System Metrics Column for Column Family - {self.stage_system_metrics_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._stage_system_metrics_columns.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Stage System Metrics Column for Column Family - {self.stage_system_metrics_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Stage System Metrics Column for Column Family - {self.stage_system_metrics_column_family_id} - if not exists"
            )
    ...
    async def submit_streams(self, stream_metrics: Dict[str, StageStreamsSet]):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Streams to Table {self.streams_table_id}"
        )
        self._streams_table = self.instance.table(self.streams_table_id)

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Streams Column Family - {self._streams_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(self._executor, self._streams_table.create)

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Streams Column Family - {self._streams_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Streams Column Family - {self._streams_column_family_id} - if not exists"
            )
        ...
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Streams Column for Column Family - {self._streams_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._streams_table_columns.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Streams Column for Column Family - {self._streams_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Streams Column for Column Family - {self._streams_column_family_id} - if not exists"
            )
    ...
    async def submit_experiments(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Experiments to Table {self.experiments_table_id}"
        )
        self._experiments_table = self.instance.table(self.experiments_table_id)

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Experiments Column Family - {self._experiments_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._experiments_table.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Experiments Column Family - {self._experiments_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Experiments Column Family - {self._experiments_column_family_id} - if not exists"
            )
        ...
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Experiments Column for Column Family - {self._experiments_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._experiments_table_columns.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Experiments Column for Column Family - {self._experiments_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Experiments Column for Column Family - {self._experiments_column_family_id} - if not exists"
            )
        ...
        await self._loop.run_in_executor(
            self._executor, self._experiments_table.mutate_rows, rows
        )

    async def submit_variants(self, experiment_metrics: ExperimentMetricsCollectionSet):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Variants to Table {self.variants_table_id}"
        )
        self._variants_table = self.instance.table(self.variants_table_id)

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Variants Column Family - {self._variants_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._variants_table.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Variants Column Family - {self._variants_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Variants Column Family - {self._variants_column_family_id} - if not exists"
            )
        ...
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Variants Column for Column Family - {self._variants_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._variants_table_columns.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Variants Column for Column Family - {self._variants_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Variants Column for Column Family - {self._variants_column_family_id} - if not exists"
            )
    ...
    async def submit_mutations(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Mutations to Table {self.mutations_table_id}"
        )
        self._mutations_table = self.instance.table(self.mutations_table_id)

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Mutations Column Family - {self._mutations_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._mutations_table.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Mutations Column Family - {self._mutations_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Mutations Column Family - {self._mutations_column_family_id} - if not exists"
            )
        ...
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Mutations Column for Column Family - {self._mutations_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._mutations_table_columns.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Mutations Column for Column Family - {self._mutations_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Mutations Column for Column Family - {self._mutations_column_family_id} - if not exists"
            )
        ...
        await self._loop.run_in_executor(
            self._executor, self._mutations_table.mutate_rows, rows
        )

    async def submit_events(self, events: List[BaseProcessedResult]):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Events to Table {self.events_table_id}"
        )
        self._events_table = self.instance.table(self.events_table_id)

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Events Column Family - {self._events_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(self._executor, self._events_table.create)

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Events Column Family - {self._events_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Events Column Family - {self._events_column_family_id} - if not exists"
            )
        ...
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Creating Events Column for Column Family - {self._events_column_family_id} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor, self._events_table_columns.create
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Created Events Column for Column Family - {self._events_column_family_id} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Skipping creation of Events Column for Column Family - {self._events_column_family_id} - if not exists"
            )
        ...
        await self._loop.run_in_executor(
            self._executor, self._events_table.mutate_rows, rows
        )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Events to Table {self.events_table_id}"
        )
