...
class AWSTimestream:
    ...
    async def submit_session_system_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saving Session System Metrics to table - {self.session_system_metrics_table_name}"
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating table - Database: {self.database_name} - Table: {self.session_system_metrics_table_name} - if not exists"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created table - Database: {self.database_name} - Table: {self.session_system_metrics_table_name} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of table - Database: {self.database_name} - Table: {self.session_system_metrics_table_name} - if not exists"
            )
        ...
        records = []
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Session System Metrics - Database: {self.database_name} - Table: {self.session_system_metrics_table_name} - if not exists"
        )
        ...
        for metrics_set in metrics_sets:
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Submitting Session System Metrics - {metrics_set.stage}:{stream_id}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Session System Metrics - Database: {self.database_name} - Table: {self.session_system_metrics_table_name} - if not exists"
        )
    ...
    async def submit_stage_system_metrics(
        ...
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating table - Database: {self.database_name} - Table: {self.stage_system_metrics_table_name} - if not exists"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created table - Database: {self.database_name} - Table: {self.stage_system_metrics_table_name} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of table - Database: {self.database_name} - Table: {self.stage_system_metrics_table_name} - if not exists"
            )
        ...
        records = []
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Stage System Metrics - Database: {self.database_name} - Table: {self.stage_system_metrics_table_name} - if not exists"
        )
        ...
        for metrics_set in metrics_sets:
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Submitting Stage System Metrics - {metrics_set.stage}:{stream_id}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Stage System Metrics - Database: {self.database_name} - Table: {self.stage_system_metrics_table_name} - if not exists"
        )

    async def submit_streams(self, stream_metrics: Dict[str, StageStreamsSet]):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saving Streams to table - {self.streams_table_name}"
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating table - Database: {self.database_name} - Table: {self.streams_table_name} - if not exists"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created table - Database: {self.database_name} - Table: {self.streams_table_name} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of table - Database: {self.database_name} - Table: {self.streams_table_name} - if not exists"
            )

        records = []
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Streams - Database: {self.database_name} - Table: {self.streams_table_name} - if not exists"
        )
        ...
        for stage_name, stream in stream_metrics.items():
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Submitting Stream - {stage_name}:{stream_id}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Stream - Database: {self.database_name} - Table: {self.streams_table_name} - if not exists"
        )
    ...
    async def submit_variants(self, experiment_metrics: ExperimentMetricsCollectionSet):
        ...
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating table - Database: {self.database_name} - Table: {self.variants_table_name} - if not exists"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created table - Database: {self.database_name} - Table: {self.variants_table_name} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of table - Database: {self.database_name} - Table: {self.variants_table_name} - if not exists"
            )

        records = []
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Variants - Database: {self.database_name} - Table: {self.variants_table_name} - if not exists"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Variants - Database: {self.database_name} - Table: {self.variants_table_name} - if not exists"
        )
    ...
    async def submit_mutations(
        ...
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating table - Database: {self.database_name} - Table: {self.mutations_table_name} - if not exists"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created table - Database: {self.database_name} - Table: {self.mutations_table_name} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of table - Database: {self.database_name} - Table: {self.mutations_table_name} - if not exists"
            )

        records = []
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Mutations - Database: {self.database_name} - Table: {self.mutations_table_name} - if not exists"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Mutations - Database: {self.database_name} - Table: {self.mutations_table_name} - if not exists"
        )

    async def submit_events(self, events: List[BaseProcessedResult]):
        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating table - Database: {self.database_name} - Table: {self.events_table_name} - if not exists"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created table - Database: {self.database_name} - Table: {self.events_table_name} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of table - Database: {self.database_name} - Table: {self.events_table_name} - if not exists"
            )

        records = []
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Events - Database: {self.database_name} - Table: {self.events_table_name} - if not exists"
        )

        for event in events:
            await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                f"{self.metadata_string} - Submitting Event - {event.name}:{event.event_id}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Events - Database: {self.database_name} - Table: {self.events_table_name} - if not exists"
        )
