...
class BigQuery:
    ...
    async def submit_session_system_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Session System Metrics - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.session_system_metrics_table_name} - if not exists"
        )
        ...
        for metrics_set in system_metrics_sets:
            ...
            if self._session_system_metrics_table is None:
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Creating table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.session_system_metrics_table_name} - if not exists"
                )
                ...
                for quantile in metrics_set.quantiles:
                    table_schema.append(bigquery.SchemaField(quantile, "FLOAT64"))
                ...
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Created table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.session_system_metrics_table_name} - if not exists"
                )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Metrics - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.session_system_metrics_table_name} - if not exists"
        )
    ...
    async def submit_stage_system_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Stage System Metrics - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.stage_system_metrics_table_name} - if not exists"
        )
        ...
        for metrics_set in system_metrics_sets:
            ...
            if self._stage_system_metrics_table is None:
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Creating table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.stage_system_metrics_table_name} - if not exists"
                )
                ...
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Created table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.stage_system_metrics_table_name} - if not exists"
                )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Metrics - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.stage_system_metrics_table_name} - if not exists"
        )

    async def submit_streams(self, stream_metrics: Dict[str, StageStreamsSet]):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Streams - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.stream_metrics_table_name} - if not exists"
        )
        ...
        for stage_name, stream in stream_metrics.items():
            ...
            if self._streams_table is None:
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Creating table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.stream_metrics_table_name} - if not exists"
                )
                ...
                streams_table = bigquery.Table(table_reference, schema=table_schema)

                await self._loop.run_in_executor(
                    self._executor,
                    functools.partial(
                        self.client.create_table, streams_table, exists_ok=True
                    ),
                )
                ...
                await self.logger.filesystem.aio["hyperscale.reporting"].info(
                    f"{self.metadata_string} - Created table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.stream_metrics_table_name} - if not exists"
                )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Metrics - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.stream_metrics_table_name} - if not exists"
        )
    ...
    async def submit_experiments(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saving Experiments to table - {self.experiments_table_name}"
        )

        if self._experiments_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.experiments_table_name} - if not exists"
            )

            experiments_table_name = (
                f"{self.project_name}.{self.dataset_name}.{self.experiments_table_name}"
            )
            ...
            table_schema = bigquery.Table(
                ...
                schema=[
                    ...
                    bigquery.SchemaField(
                        "experiment_randomized", "BOOLEAN", mode="REQUIRED"
                    ),
                    bigquery.SchemaField(
                        "experiment_completed", "INT64", mode="REQUIRED"
                    ),
                    bigquery.SchemaField(
                        "experiment_succeeded", "INT64", mode="REQUIRED"
                    ),
                    bigquery.SchemaField("experiment_failed", "INT64", mode="REQUIRED"),
                    bigquery.SchemaField(
                        "experiment_median_aps", "FLOAT64", mode="REQUIRED"
                    ),
                ],
            )
            ...
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    self.client.create_table, self._experiments_table, exists_ok=True
                ),
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.experiments_table_name} - if not exists"
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Experiments to - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.experiments_table_name}"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Experiments to - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.experiments_table_name}"
        )

    async def submit_variants(self, experiment_metrics: ExperimentMetricsCollectionSet):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saving Variants to table - {self.variants_table_name}"
        )

        if self._variants_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.variants_table_name} - if not exists"
            )

            variants_table_name = (
                f"{self.project_name}.{self.dataset_name}.{self.variants_table_name}"
            )
            ...
            table_schema = bigquery.Table(
                ...
                schema=[
                    ...
                    bigquery.SchemaField(
                        "variant_experiment", "STRING", mode="REQUIRED"
                    ),
                    bigquery.SchemaField("variant_weight", "FLOAT64", mode="REQUIRED"),
                    bigquery.SchemaField(
                        "variant_distribution", "STRING", mode="REQUIRED"
                    ),
                    bigquery.SchemaField(
                        "variant_distribution_interval", "FLOAT64", mode="REQUIRED"
                    ),
                    bigquery.SchemaField(
                        "variant_ratio_completed", "FLOAT64", mode="REQUIRED"
                    ),
                    bigquery.SchemaField(
                        "variant_ratio_succeeded", "FLOAT64", mode="REQUIRED"
                    ),
                    bigquery.SchemaField(
                        "variant_ratio_failed", "FLOAT64", mode="REQUIRED"
                    ),
                    bigquery.SchemaField(
                        "variant_ratio_aps", "FLOAT64", mode="REQUIRED"
                    ),
                    ...
                    bigquery.SchemaField(
                        "variant_actions_per_second", "FLOAT64", mode="REQUIRED"
                    ),
                ],
            )
            ...
            self._variants_table = bigquery.Table(table_reference, schema=table_schema)

            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    self.client.create_table, self._variants_table, exists_ok=True
                ),
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.variants_table_name} - if not exists"
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Variants to - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.variants_table_name}"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Variants to - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.variants_table_name}"
        )
    ...
    async def submit_mutations(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Saving Mutations to table - {self.mutations_table_name}"
        )

        if self._mutations_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.mutations_table_name} - if not exists"
            )
            ...
            table_schema = bigquery.Table(
                ...
                schema=[
                    ...
                    bigquery.SchemaField(
                        "mutation_experiment_name", "STRING", mode="REQUIRED"
                    ),
                    bigquery.SchemaField(
                        "mutation_variant_name", "STRING", mode="REQUIRED"
                    ),
            ...
            self._mutations_table = bigquery.Table(table_reference, schema=table_schema)

            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    self.client.create_table, self._mutations_table, exists_ok=True
                ),
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.mutations_table_name} - if not exists"
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Mutations to - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.mutations_table_name}"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Mutations to - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.mutations_table_name}"
        )

    async def submit_events(self, events: List[BaseProcessedResult]):
        if self._events_table is None:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.events_table_name} - if not exists"
            )

            events_table_name = (
                f"{self.project_name}.{self.dataset_name}.{self.events_table_name}"
            )
            ...
            self._events_table = bigquery.Table(table_reference, schema=table_schema)

            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    self.client.create_table, self._events_table, exists_ok=True
                ),
            )

            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created table - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.events_table_name} - if not exists"
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Events to - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.events_table_name}"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Events to - Project: {self.project_name} - Dataset: {self.dataset_name} - Table: {self.events_table_name}"
        )
