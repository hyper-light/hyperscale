...
class NewRelic:
    ...
    async def submit_session_system_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Session System Metrics to NewRelic"
        )
        ...
        for metrics_set in metrics_sets:
            ...
            for field, value in metrics_set.record.items():
                ...
                await self._loop.run_in_executor(
                    self._executor,
                    functools.partial(
                        self.client.record_custom_metric, record_name, value
                    ),
                )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Session System Metrics to NewRelic"
        )
    ...
    async def submit_stage_system_metrics(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Stage System Metrics to NewRelic"
        )
    ...
    async def submit_streams(self, stream_metrics: Dict[str, StageStreamsSet]):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Streams to NewRelic"
        )
        ...
        for stage_name, stream in stream_metrics.items():
            ...
            for group_name, group in stream.grouped.items():
                for field, value in group.items():
                    await self.logger.filesystem.aio["hyperscale.reporting"].debug(
                        f"{self.metadata_string} - Submitting Streams - {stage_name}:{group_name}:{field}"
                    )
                    await self._loop.run_in_executor(
                        self._executor,
                        functools.partial(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Streams to NewRelic"
        )
    ...
    async def submit_experiments(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Experiments to NewRelic"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Experiments to NewRelic"
        )
    ...
    async def submit_variants(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Variants to NewRelic"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Variants to NewRelic"
        )
    ...
    async def submit_mutations(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Mutations to NewRelic"
        )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Mutations to NewRelic"
        )

    async def submit_events(self, events: List[BaseProcessedResult]):
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Events to NewRelic"
        )
        ...
        for event in events:
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
                    self.client.record_custom_event, event.name, event.record
                ),
            )

        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Events to NewRelic"
        )
