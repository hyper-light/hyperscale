...
class S3:
    ...
    async def submit_session_system_metrics(
        ...
        session_system_metrics_bucket_name = (
            f"{self.buckets_namespace}-{self.session_system_metrics_bucket_name}"
        )
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Session System Metrics to Bucket - {session_system_metrics_bucket_name}"
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Session System Metrics Bucket - {session_system_metrics_bucket_name} - if not exists"
            )
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Session System Metrics Bucket - {session_system_metrics_bucket_name}"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of Session System Metrics Bucket - {session_system_metrics_bucket_name}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Session System Metrics to Bucket - {session_system_metrics_bucket_name}"
        )

    async def submit_stage_system_metrics(
        self, system_metrics_sets: List[SystemMetricsSet]
    ):
        stage_system_metrics_bucket_name = (
            f"{self.buckets_namespace}-{self.stage_system_metrics_bucket_name}"
        )
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Stage System Metrics to Bucket - {stage_system_metrics_bucket_name}"
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Stage System Metrics Bucket - {stage_system_metrics_bucket_name} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Stage System Metrics Bucket - {stage_system_metrics_bucket_name}"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of Stage System Metrics Bucket - {stage_system_metrics_bucket_name}"
            )
        ...
        for metrics_set in metrics_sets:
            ...
            metrics_set_set_key = (
                f"{metrics_set.name}_{metrics_set.group}_{stage_system_metrics_set_id}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Stage System Metrics to Bucket - {stage_system_metrics_bucket_name}"
        )

    async def submit_streams(self, stream_metrics: Dict[str, StageStreamsSet]):
        streams_bucket_name = f"{self.buckets_namespace}-{self.streams_bucket_name}"
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Streams to Bucket - {streams_bucket_name}"
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Streams Bucket - {streams_bucket_name} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Streams Bucket - {streams_bucket_name}"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of Streams Bucket - {streams_bucket_name}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Streams to Bucket - {streams_bucket_name}"
        )
    ...
    async def submit_experiments(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Experiments to Bucket - {experiments_bucket_name}"
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Experiments Bucket - {experiments_bucket_name} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Experiments Bucket - {experiments_bucket_name}"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of Experiments Bucket - {experiments_bucket_name}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Experiments to Bucket - {experiments_bucket_name}"
        )
    ...
    async def submit_variants(self, experiment_metrics: ExperimentMetricsCollectionSet):
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Variants to Bucket - {variants_bucket_name}"
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Variants Bucket - {variants_bucket_name} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Variants Bucket - {variants_bucket_name}"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of Variants Bucket - {variants_bucket_name}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Variants to Bucket - {variants_bucket_name}"
        )
    ...
    async def submit_mutations(
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Mutations to Bucket - {mutations_bucket_name}"
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Mutations Bucket - {mutations_bucket_name} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Mutations Bucket - {mutations_bucket_name}"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of Mutations Bucket - {mutations_bucket_name}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Mutations to Bucket - {mutations_bucket_name}"
        )

    async def submit_events(self, events: List[BaseProcessedResult]):
        events_bucket_name = f"{self.buckets_namespace}-{self.events_bucket_name}"
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitting Events to Bucket - {events_bucket_name}"
        )

        try:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Creating Events Bucket - {events_bucket_name} - if not exists"
            )
            await self._loop.run_in_executor(
                self._executor,
                functools.partial(
            ...
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Created Events Bucket - {events_bucket_name} - if not exists"
            )

        except Exception:
            await self.logger.filesystem.aio["hyperscale.reporting"].info(
                f"{self.metadata_string} - Skipping creation of Events Bucket - {events_bucket_name}"
            )
        ...
        await self.logger.filesystem.aio["hyperscale.reporting"].info(
            f"{self.metadata_string} - Submitted Events to Bucket - {events_bucket_name}"
        )
