...
from hyperscale.reporting.metric.metric_types import MetricType

CalculationMethod = Callable[
    [Dict[str, List[Union[int, float]]]], Dict[str, Union[int, float]]
]


class StageStreamsSet:
    def __init__(self, stage: str, stage_streams: List[StreamAnalytics]) -> None:
        self.stage = stage
        ...
        self.quantiles = [10, 20, 25, 30, 40, 50, 60, 70, 75, 80, 90, 95, 99]
    ...
    def grouped(self) -> Dict[str, Dict[str, Union[int, float]]]:
        ...
        for group_name, group_stats in self.stats.items():
            metrics: Dict[str, Union[int, float]] = {
                stat_name: calculation_method(group_stats)
                for stat_name, calculation_method in self._stat_types.items()
            }

            metrics.update(self._calculate_quantiles(group_stats))
    ...
    def record(self) -> Dict[str, Union[int, float]]:
        ...
        actions_per_second_succeeded = (
            record_stats["median_succeeded"] / median_batch_time
        )
    ...
    def _calculate_quantiles(
        ...
        for quantile in self.quantiles:
            ...
            quantile_stats[stat_key] = numpy.quantile(stats, round(quantile / 100, 2))
