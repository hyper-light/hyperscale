...
from .metrics_set_types import CommonMetrics, GroupMetricsSet, StageMetrics
...
class StageMetricsSummary:
    def __init__(
        ...
        if self.stage_streamed_analytics:
            initial_stream = [
                0
                for _ in range(
                    len(self.stage_streamed_analytics[0].interval_completed_counts)
                )
            ]
        ...
        self.common_fields: List[str] = ["total", "succeeded", "failed"]
        ...
        for group in self.groups:
            for field in self.fields:
                self.action_or_task_header_keys.append(f"{group}_{field}")
    ...
    @property
    def streams(self):
        return StageStreamsSet(self.stage_metrics.name, self.stage_streamed_analytics)
    ...
    def _group_action_and_task_metrics(self):
        for action_or_task_name, metric_set in self.metrics_sets.items():
            ...
            self.action_and_task_metrics[action_or_task_name] = GroupMetricsSet(
                **grouped_metrics
            )

    def _calculate_stage_totals(self):
        self.stage_metrics.aps = round(
            self.stage_metrics.total / self.stage_metrics.time, 2
        )
    ...
    def _calculate_completion_rate_stream(self):
        bins_count = max(
            len(stream.interval_completion_rates)
        ...
        for idx in range(bins_count):
            self.stage_metrics.streamed_completion_rates[idx] = round(
                self.stage_metrics.streamed_completed[idx]
                / self.stage_metrics.streamed_batch_timings[idx],
                2,
            )

    def _calculate_succeeded_rate_stream(self):
        bins_count = max(
            len(stream.interval_succeeded_counts)
            for stream in self.stage_streamed_analytics
        )
    ...
    def _calculate_failed_rate_stream(self):
        bins_count = max(
            len(stream.interval_failed_counts)
            for stream in self.stage_streamed_analytics
        )
    ...
    def _calculate_completed_rate_stream(self):
        bins_count = max(
            [
                len(stream.interval_completed_counts)
                for stream in self.stage_streamed_analytics
            ]
        )
    ...
    def _calculate_batch_timings_stream(self):
        bins_count = max(
            [
                len(stream.interval_batch_timings)
                for stream in self.stage_streamed_analytics
            ]
        )
        ...
        for idx, batch_timing in enumerate(streamed_batch_timings):
            streamed_batch_timings[idx] = round(batch_timing, 2)
