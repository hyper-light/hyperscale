...
from hyperscale.reporting.tags import Tag
...
class MetricsSet:
    def __init__(
        ...
        self.tags = [Tag(tag_name, tag_value) for tag_name, tag_value in tags.items()]

        metrics_groups = metrics_data.get("groups", {})
        for group_name, group in metrics_groups.items():
            self.groups[group_name] = MetricsGroup(
                name, source, stage, group_name, group, self.common_stats
            )

        record_fields = self.groups.get("total").fields
        custom_fields = list(self.groups.get("total").custom_fields.keys())
        ...
        self.quantiles = list(self.groups.get("total").quantiles.keys())

        self.custom_field_names = self.custom_fields = self.groups.get(
            "total"
        ).custom_field_names

        self.custom_fields = self.groups.get("total").custom_fields

        self.custom_schemas = self.groups.get("total").custom_schemas

        self.stats_fields = list(self.groups.get("total").stats.keys())
