...
class SessionMetricsCollection(BaseModel):
    ...
    def record(self) -> Dict[str, Union[str, bool, int, float]]:
        return {"name": self.name, "group": self.group, **self.stats}
    ...
    @property
    def tags(self):
        tag_fields = {"name": self.name, "group": self.group}

        return [
            Tag(tag_field_name, tag_field_value)
...
class SystemMetricsCollection(BaseModel):
    ...
    @property
    def tags(self):
        tag_fields = {"stage": self.stage, "name": self.name, "group": self.group}
