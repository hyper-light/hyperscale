    ...
    def aggregate(self):
        for stage_name, metrics in self.raw_metrics.items():
            for monitor_name, monitor_metrics in metrics.collected.items():
                self.visibility_filters[stage_name][monitor_name] = (
                    metrics.visibility_filters[monitor_name]
                )
                ...
                if len(metrics_data) > 0:
                    self.stage_metrics[stage_name][monitor_name] = (
                        metrics.stage_metrics[monitor_name]
                    )
                        ...
                        **{
                            ...
                            **{
                                f"quantile_{quantile}th": numpy.quantile(
                                    metrics_data, round(quantile / 100, 2)
                                )
