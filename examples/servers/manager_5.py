#!/usr/bin/env python3
"""
Manager Test Server 5

This server runs on ports 9008 (TCP) and 9009 (UDP) and demonstrates
...
async def run_manager_5():
    ...
    print(f"\n[Manager 5] Started! Node ID: {server._node_id.short}")
    
    try:
        while True:
            await asyncio.sleep(5)
            print(f"\n[Manager 5] State={server._manager_state.value}, Leader={server.is_leader()}, Workers={len(server._workers)}")
...
if __name__ == '__main__':
    try:
        asyncio.run(run_manager_5())
    except KeyboardInterrupt:
        print("\n[Manager 5] Interrupted")
