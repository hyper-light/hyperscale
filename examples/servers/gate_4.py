...
This server runs on ports 9106 (TCP) and 9107 (UDP) and demonstrates
...
async def run_gate_4():
    ...
    print(f"\n[Gate 4] Started! Node ID: {server._node_id.short}")
    
    try:
        while True:
            await asyncio.sleep(5)
            print(f"\n[Gate 4] State={server._gate_state.value}, Leader={server.is_leader()}, Jobs={len(server._jobs)}")
