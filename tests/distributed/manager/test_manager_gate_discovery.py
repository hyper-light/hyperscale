...
async def scenario_manager_gate_discovery_single_dc(
    gate_count: int,
    manager_count: int,
) -> bool:
    """
...
async def scenario_manager_gate_discovery_multi_dc(
    gate_count: int,
    managers_per_dc: int,
    dc_count: int,
) -> bool:
...
async def scenario_manager_gate_discovery_failure_recovery(
    gate_count: int,
    manager_count: int,
) -> bool:
    """
...
async def scenario_manager_gate_message_validation(gate_count: int, manager_count: int) -> bool:
    """
...
async def run_all_tests():
    ...
    for gates, managers in [(2, 2), (3, 3), (3, 5)]:
        result = await scenario_manager_gate_discovery_single_dc(gates, managers)
    ...
    for gates, managers_per_dc, dcs in [(2, 2, 2), (3, 3, 2), (3, 2, 3)]:
        result = await scenario_manager_gate_discovery_multi_dc(gates, managers_per_dc, dcs)
        results[f"multi_dc_{gates}g_{managers_per_dc}m_{dcs}dc"] = result
        await asyncio.sleep(2)

    # Message validation tests
    print("\n--- Message Validation Tests ---")
    result = await scenario_manager_gate_message_validation(2, 3)
    ...
    for gates, managers in [(2, 3), (3, 3)]:
        result = await scenario_manager_gate_discovery_failure_recovery(gates, managers)
