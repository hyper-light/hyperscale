...
KEYWORD_REQUIREMENTS = {
    "dispatch": ["_job_router", "_job_dc_managers"],
    "routing": ["_job_router", "_coordinate_tracker", "_blended_scorer"],
    "forward": ["_job_forwarding_tracker"],
    "idempotency": ["_idempotency_cache"],
    "register": ["_datacenter_manager_status", "_manager_health"],
    "registration": ["_datacenter_manager_status", "_manager_health"],
    "discovery": ["_datacenter_manager_status", "_manager_negotiated_caps"],
    "heartbeat": ["_manager_last_status"],
    "health": ["_manager_health"],
    "overload": ["_overload_detector", "_load_shedder"],
    ...
    "capacity": ["_capacity_aggregator"],
    "spillover": ["_capacity_aggregator"],
    "progress": [
        "_job_progress_sequences",
        "_job_progress_seen",
        "_progress_callbacks",
    ],
    "stats": ["_job_stats_crdt"],
    "workflow result": ["_workflow_dc_results", "_job_workflow_ids"],
    "result": ["_workflow_dc_results", "_job_workflow_ids"],
    "final": ["_job_manager", "_dispatch_time_tracker", "_observed_latency_tracker"],
    "timeout": ["_job_timeout_tracker"],
    "reporter": ["_job_reporter_tasks"],
    "leadership": ["_job_leadership_tracker"],
    "leader": ["_job_leadership_tracker"],
    "lease": ["_leases", "_fence_token"],
    "fence": ["_fence_token"],
    "quorum": ["_quorum_circuit"],
    "sync": ["_state_sync_handler", "_state_version"],
    "snapshot": ["_state_sync_handler", "_state_version"],
    "protocol": ["_manager_negotiated_caps"],
    "capabilit": ["_manager_negotiated_caps"],
    "negotiat": ["_manager_negotiated_caps"],
    "cancel": ["_cancellation_errors", "_cancellation_completion_events"],
    "cancellation": ["_cancellation_errors", "_cancellation_completion_events"],
    "throughput": ["_forward_throughput_count", "_forward_throughput_interval_start"],
    "latency": ["_forward_throughput_interval_start"],
    "error": ["_load_shedder", "_rate_limiter"],
    "exception": ["_load_shedder", "_rate_limiter"],
}
...
def _extract_section_bullets(section_number: int) -> list[str]:
    ...
    for line in SCENARIO_PATH.read_text().splitlines():
        if SECTION_START in line:
            ...
            continue
        if in_gate_section and SECTION_END in line:
            break
...
def _build_title(section_number: int, bullet: str) -> str:
    return f"Gate<->Manager {section_number}: {bullet}"
...
async def run_section(section_number: int) -> None:
    ...
    if not bullets:
        raise AssertionError(
            f"No bullets found for Gate<->Manager section {section_number}"
        )
